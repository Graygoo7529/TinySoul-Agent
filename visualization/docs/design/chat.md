...
- Agent 行在完成态展示 Markdown 渲染的最终回答。回答卡为微渐变亮面纸感（顶部内高光 + accent 淡彩柔影）；当前会话内完成时，回答在**状态卡折叠完成后**以固定节奏（约 330 字/秒，6s 封顶）逐字流入，流入期间卡片带 accent 柔光与顶部流光扫掠（`answer-streaming`），历史恢复的回答直接全量呈现。页脚给出状态徽标、耗时、概要（cycles/actions/domains/成败数）、token 用量与 Details 入口。
...
- **沉淀态（settled 折叠条）**：LiveStatus 在运行与落定之间是**同一卡片实例**——轮次结束时卡体（思考面板 + 步骤区 + 工作区）从下沿向上卷起折叠（550ms 高度 tween），抬头淡换为落定状态行（"回答完成/轮次失败…" + 概要 + 定格总时长），呼吸边框转静态；折叠条带 chevron，点击重新展开完整静态轨迹（类似原沉淀卡）。发起新 user turn 时旧折叠条自然收起（新一轮挂出自己的运行卡）。**终态清扫**：轮次结束时 derive 层把遗留 running phase 置 `ended`、未落定 action 条目置 `stopped`（静态停止图标）、in_progress todo 置 `cancelled`——settled 卡与 trace 抽屉里不再有任何"还在转"的假象（无结果的 ActionCard 显示 interrupted 徽章）。
...
- **顶部停泊跟随（ChatView）**：活跃轮次（运行中或回答流入中）的用户气泡停泊在视口顶沿下 20px（`TOP_ANCHOR`）——新一轮开始时，运行中占位空间（约 85vh 的 spacer）保证锚点可达，750ms 平滑滑动把用户气泡送到顶沿；随后 ResizeObserver 逐帧跟随：卡片向下扩展、折叠、回答流入都不动顶部。轮次内容超出视口时平滑交接为钉底跟随（保证流入中的回答尾部可见）。用户滚轮/触摸立即接管（滑动即时取消，滚动到底部附近自动重新停泊）；回答流入结束后视图自然释放。
