...
- **流动更迭节奏**：900ms 原子节拍（`useThrottledValue`）把抬头与步骤轨迹合并为同一次提交，爆发期合并为一拍、尾随冲刷保证最终状态不丢；每拍分两个相位——抬头与思考流先经 `Crossfade` 交叉淡入淡出（旧内容绝对定位淡出、新内容淡入，高度差平滑收放），约 200ms 后步骤轨迹柔和入场（纯淡入 0.45s + 70ms 级联、上限 3 级）；新条目、glimpse 与工作区首次出现均经 `grow-in` 高度展开（grid 0fr→1fr，0.45s 平缓腾位、无布局跳变）。
...
- **思考流**：最新一条 reasoning summary 自动展开，新思考经 `Crossfade` 交叉淡入淡出并平滑收放高度；整块首次出现时经 `grow-in` 展开入卡。
- **语义步骤栈**：最新在前的活动条目（intent + domain 胶囊、挂载 skills、todo/milestone、action），相位延迟入场、纵深渐隐（`.step-depth` 过渡）。运行中步骤区为**固定最大高度的滚动视口**（`.steps-viewport`，约 5 行，渲染窗口 7 条）：新条目把旧条目整体平滑下推，抵近底边的旧条目在渐变消融 mask 中淡出消失（仅溢出时启用，ResizeObserver 判定），视口填满后卡片高度恒定；溢出条目折叠为 "+N earlier steps"，展开时解除高度限制（用户主动行为）。配合 `ChatView` 的 ResizeObserver 逐帧钉底，页面底部不再随更新瞬跳。action 条目是状态机（planned 空心点 → running 旋转 → succeeded/failed/timeout），并呈现两条信息：上行 stage2 调用语义（编辑的目标文件、检索意图、待访问链接、待执行命令、inspect/recall 目标），下行 stage3 结果摘要（exit code、结果数、revision、行数）。
