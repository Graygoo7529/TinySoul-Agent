...
- **思考流**：最新一条 reasoning 以**固定一行**呈现（`.thinking-slate`）——内容为第一个非空行的**行内 Markdown 预览**（`.md-inline`：块元素降为行内、加粗压平忽略、链接惰性、数学经 KaTeX 行内排版、省略号截断；整体以 11px/340 细体微斜（oblique 8°）+ fg-faint 呈现，左端留 4px 空隙），换思考时旧行淡出、新行柔和浮现，面板高度恒定不动；有更多内容时可 Expand 展开为完整 Markdown（高度平滑释放）。整块首次出现时经 `grow-in` 展开入卡。
