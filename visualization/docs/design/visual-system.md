...
- 运行态统一 `pulse-dot` / `spin-slow`；LiveStatus 专属 `text-shine`（2.6s 流光）、`live-border`（2.8s 呼吸）、`status-in`（0.28s 上浮）、`answer-in`（0.5s 浮现）、`xfade`（0.28/0.36s 交叉淡入淡出 + 0.36s 高度收放，`Crossfade` 承载抬头与 glimpse 交换）、`grow-in`（0.45s 高度展开，grid 0fr→1fr，reduced-motion 下直落终态）；步骤滚动、思考打字机与就地更新由 **motion 库**驱动（height auto tween、`AnimatePresence` 退出/坍缩、reduced-motion 经 `useReducedMotion` 直落终态），节拍时间线详见 chat.md；思考面板 `.thinking-slate`（隐藏副本预选测量高度 + 底边截断消融）与步骤视口 `.steps-viewport`（12rem 固定最大高度 + 溢出闩锁 + 底边 2.5rem 渐变消融）配合 `ChatView` 顶部停泊跟随（`TOP_ANCHOR`）共同稳定卡片呼吸边框；活动 feed 经 1500ms 原子节拍合并并发爆发（`useThrottledValue`，抬头与步骤同拍提交）。
