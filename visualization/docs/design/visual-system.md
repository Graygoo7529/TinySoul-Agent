...
- **Markdown**（`.md-body`）：紧凑标题层级、行内 code 灰底 accent 字、pre 圆角 code-bg、表格/引用/任务列表齐全；数学经 remark-math + KaTeX 排版（`$行内$` / `$$块级$$` / `\(...\)`，KaTeX 字体随包体分发）。变体：`.md-calm` 柔化加粗为 500；`.md-inline` 单行预览（思考面板折叠态——块元素行内化、字重压平（加粗忽略）、链接惰性、数学行内尺寸且正立、省略号截断；整行 11.5px/380 细体微斜 oblique 8° + fg-faint，左端 12px 留白）；`.thinking-md` 展开态思考排版（与预览同字号同风格，fg-muted 更易读，加粗 500、斜体记号正立、公式正立）。

## 动效

- `fade-in`（0.18s）用于弹层与消息进入；`slide-in-right`（0.22s）用于抽屉；drawer 用 0.24s cubic-bezier 推入。
- 运行态统一 `pulse-dot` / `spin-slow`；LiveStatus 专属 `text-shine`（2.6s 流光）、`live-border`（2.8s 呼吸）、`answer-in`（0.5s 浮现）、`xfade`（0.28/0.36s 交叉淡入淡出 + 0.36s 高度收放，`Crossfade` 承载抬头与 glimpse 交换）、`grow-in`（0.45s 高度展开，grid 0fr→1fr，reduced-motion 下直落终态）；步骤滚动、思考行浮现、最终回答打字机（`useTypewriter`，时长按长度缩放约 0.5–2s）与就地更新由 **motion 库**驱动（height auto tween、`AnimatePresence` 退出/坍缩、reduced-motion 经 `useReducedMotion` 直落终态），节拍时间线详见 chat.md；思考面板 `.thinking-slate`（固定一行，Expand 平滑释放）与步骤视口 `.steps-viewport`（12rem 固定最大高度 + 溢出闩锁 + 底边 2.5rem 渐变消融）配合 `ChatView` 顶部停泊跟随（`TOP_ANCHOR`）共同稳定卡片呼吸边框；活动 feed 经 1500ms 原子节拍合并并发爆发（`useThrottledValue`，抬头与步骤同拍提交）；activity 条目携带稳定 `seq` 与源事件时间戳，全量重建不重挂载、时间不漂移。
