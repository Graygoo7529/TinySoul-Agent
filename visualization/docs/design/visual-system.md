...
- **Markdown**（`.md-body`）：紧凑标题层级、行内 code 灰底 accent 字、pre 圆角 code-bg、表格/引用/任务列表齐全；数学经 remark-math + KaTeX 排版（`$行内$` / `$$块级$$` / `\(...\)`，KaTeX 字体随包体分发）。变体：`.md-calm` 柔化加粗为 500；`.md-inline` 单行预览（思考面板折叠态——块元素行内化、字重字压平（加粗忽略）、链接惰性、数学行内尺寸、省略号截断；整行 11px/340 细体微斜 oblique 8°）。
