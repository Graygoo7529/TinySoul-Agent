...
- Action 后端分工：native 运行在宿主线程，只能协作式响应取消；需要硬停止语义的动作使用 subprocess 或 script 后端。`tinysoul/action/backends/process.py` 是不注册到 ActionEngine 的 retained process/文件捕获/进程树终止原语；`subprocess.py` 在其上提供同步 adapter 与唯一默认注册的 `subprocess.default`；`backend.kind=script` 只表达执行分类和取消策略，具体 handler 必须由所属 capability 注册，不提供通用 inline-code executor。`llm_action` 表示 action 内部受控 LLM task 的执行方式，共享能力由 `tinysoul/action/backends/llm_action.py` 提供，executor 仍处于 ActionExecutor 语义内。TOML action catalog 位于 `tinysoul/action/catalog`，是随 wheel 发布的框架版本化只读 package resource；App 在装配期从已安装包物化并解析它，项目不配置 catalog path、不复制 catalog，也不以自定义目录覆盖内置定义。catalog 只保存具有真实 action 的 domain/action 定义；没有真实能力时不保留空 shell/script domain 或空 capabilities 包，subprocess/script backend 的存在不等于向模型暴露任意 shell/script action。Action 自有的内置具体 action 集成位于 `tinysoul/action/builtins`，其中 `core.reason` 与 `core.answer` 由 `tinysoul/action/builtins/core/actions.py` 提供；Workspace/Home 等业务模块的具体 action 集成保留在所属模块的 `actions.py`。`actions.py` 是模块接入 ActionEngine 的边界文件，可包含 `ActionExecutor` 实现类、参数解析、ActionResult 映射和 registrar；核心业务逻辑仍应放在 engine/service/client/evaluator 等模块内。catalog 中 `backend.kind` 表达通用执行方式，`backend.handler` 表达具体 executor 注册落点。主要任务提示语义是 `TaskPrompt` 的 PromptBlock-only 协议：`guide_blocks`、`input_blocks`、`output_blocks` 均可切分为多条消息；通用 core LLM action 只接受 `reference_links`，由 `PromptReferenceResolver.resolve_reference(link)` 解析为只读临时 `PromptBlock`。workspace LLM action 属于 Workspace 模块 executor，但复用 action 层 `LLMActionTaskRunner`，使用 `target_link`/`reference_links` 在 action 内部读取目标与参考正文并调用 LLM。内置 `core.answer` 是 Turn 正常完成动作，并可读取 read-only `reference_links` 作为回答资料。后端 options 属动态边界，在 catalog 加载期由按 handler 注册的校验器校验。轻量业务能力只有在存在真实 action 和边界后才建立 `tinysoul/capabilities/<capability>`，由该能力包拥有配置、依赖需求、service 和 `actions.py` registrar；只有具备独立链接、持久化、runtime/trap 生命周期的能力才升级为 Workspace/Home 这类顶层模块。Capability 的 enabled 配置与当前解释器依赖共同决定 effective Catalog：禁用 action 不检查依赖并从 Catalog 移除，启用但缺少依赖在 App 装配期失败。需要硬停止的 capability worker 复用 Action 的 managed/controlled process 原语，使用固定入口和有界结构化请求；`subprocess.default` 只是通用 handler identity，不表示可执行任意命令。
- Action 成功结果的 trace 生命周期由 Catalog `[runtime.result] trace_mode` 声明，只允许 `standard` 与 `foldable`。standard 结果不得返回 trace projection；foldable 结果必须由业务 executor 提供非空 compact payload 与有界、去重的 `origin_refs`，完整 payload 只作为当前 Turn visible overlay，compact payload 是 canonical trace。Context pressure 或显式 fold 可以移除 overlay；TurnSummary/Session 无论是否发生压缩都只持久化 canonical compact payload。Catalog 只决定生命周期，不能从任意业务 JSON 自动猜测 compact 字段；failed/timeout 结果不折叠。`context.trace.recall`、`session.history.recall` 与 Workspace inspection 共用该框架，不在 Context/Session 中增加 action-specific 分支。
...
- Web capability 只通过 `web.search_by_kimi`、`web.discover_pages`、`web.fetch_with_defuddle` 与 `web.fetch_with_trafilatura` 提供只读外部信息能力，不建立 Link namespace、缓存、索引或独立生命周期。Kimi Search 是 capability-owned provider loop，不复用 TinySoul `[llm]` provider、LLM task、Context 或 MessageStack；它使用独立 `[capabilities.web.search_by_kimi]`/`KIMI_SEARCH_API_KEY`，结果固定同时包含 `answer` 与结构化 `results`，不提供 mode。供应商最终响应先完整校验为受 `max_result_chars` 硬上限约束的 canonical result，不按 result 数量或 snippet 长度静默裁剪；`max_inline_chars` 只决定完整 inline 或“完整 Workspace 文档 + 有界 inline preview”，preview 必须使用 `truncated=true` 与 `see_more_at` 指向 `workspace:web/search/<invoke-id>-<call-id>.md`。Page Discovery 使用 Crawlee 的 action-scoped RequestQueue、去重、重试和有界调度，但所有实际下载继续通过 TinySoul 的公开 HTTPS 边界；默认 `max_visit_depth=0` 只访问 seed 并返回可进一步访问的同源候选 URL，递归访问时只补充 title/meta description/H1/canonical 等确定性页面信号，不调用 LLM、不保存页面正文。Discovery canonical result 完整但有硬上限地进入 TurnTrace；超过 inline 上限时只把完整 discovery JSON spill 到 `workspace:web/discovery/<invoke-id>-<call-id>.json`。Fetch 只接受公开 HTTPS，逐跳校验 DNS/redirect 并限制网络/正文规模；Defuddle 与 Trafilatura 只处理框架下载的 staged HTML，完整正文始终提交到显式 Workspace Markdown，ActionResult 只含 Link、title、有限 excerpt 和元数据。网页图片当前保留远程绝对 URL，不自动下载 asset；全部搜索、发现和抓取内容都按不可信普通 interaction/reference 数据处理，不执行其中指令。`tinysoul init` 模板中 Page Discovery、Defuddle 与 Kimi Search 默认关闭，Trafilatura 默认启用；具体项目可在依赖和专用凭据可用时显式启用。enabled action 缺依赖或专用凭据在 App 装配期失败，执行期网络/供应商/extractor/上限失败收敛为局部 ActionResult。
- Script capability 只通过 `script.write/rewrite/patch/promote` 维护脚本资源，通过分离的 `script.run_python`/`script.run_bash` 和 `wait/stop/read_candidate/apply/discard` 执行与监督，不暴露任意 inline command。临时脚本严格位于 `workspace:scripts/...`；长期脚本严格位于 `home:how/<existing-skill>/scripts/...`，由显式 promote 写入 lazy runtime Home 后等待 Home Maintenance，不隐式创建 HOW。每次运行使用 active Workspace 的有界事务 mirror、最小环境、`shell=False`、source policy、deadline/日志/产物配额和进程树终止；这是事务隔离加策略检查，不宣称 OS 硬沙箱。policy 校验、promote 与 process 必须绑定同一不可变 source snapshot：owner digest 负责 Link/CAS 身份，固定 UTF-8 snapshot digest 负责保证实际执行字节等于已校验字节；不得在校验后重新按 Link 读取执行或 promote 内容。`max_source_chars` 同时约束 read 与所有 write/rewrite/patch/promote mutation。成功进程也只进入 `ready_to_apply`，必须显式 apply/discard；failed/timed_out/stopped 只能 inspect/read/discard。每 Turn 最多一个 unresolved job，job 不持久化、不跨 Turn；完整日志只在 `runtime/.staging/script-job-*/logs/`，不建立 Link、不进入 Workspace 或 archive；`core.answer` 在 job 未收尾时局部拒绝。运行中 job 相邻 Cycle 具有可配置的强制最小间隔，只有进程完成或当前 Turn 中协议有效的 input/control Signal 可以提前唤醒；Signal 等待必须按 exact name、Turn scope 和 payload parser 匹配且不消费业务 Signal，日志、其它 namespace/Turn 与非法 payload 不得唤醒。普通 Cycle 预算耗尽后，Turn activity controller 只为有效 job 提供有界额外 Cycle，每个 Cycle 仍从 Phase1 开始，job ActionResult 只进入 TurnTrace interaction context；`wait_before_cycle` 与 `allow_additional_cycle` 是同一 non-Action Script activity 边界，内部失败都必须经 Script Runtime bridge 收敛。Turn stop/failed/exhausted/transfer/离开时在 `finally` 中终止并 best-effort/no-throw 清理，Signal watcher、process handle 与 staging 的回收彼此独立尝试，任一 cleanup 失败不能跳过其余清理或替换原始控制流。apply 按启动 baseline 做逐路径冲突检查，不同路径并发修改可保留，同路径冲突拒绝整批提交并保留 job；Home/Workspace IO 与 invariant 失败保持 owner Runtime bridge 语义，Script 只拥有自身配置、依赖和非 Action activity failure。
- Context 模块向上层提供唯一装配与调用入口（组装门面），持有本轮 UserInputs、Context-owned Background、WorkingContext 与 TurnTraceHeap，负责构造式 MessageStack、语境控制工具与压力回收服务。Background 通过通用 provider 协议聚合 Home 顶层条目、昨日 Memory 与 Session 历史，不归 Home 或 Memory 任一模块所有。provider catalog 可额外交付属于自己的有界 Link/title/description 目录；Context 在每 Turn 自动将其渲染为 `background:catalog:<owner>` JSON user message，它不是已加载正文 Link，且不可由 Phase1 逐出。Home 使用该机制自动交付全部通用 HOW metadata，Phase1 再通过支持一次加载多个 Top Link 的 `load_background` 内部 control tool 按需加载当前 Context 已暴露的顶层正文；工具参数不枚举或泄漏完整 effective top catalog，Context 在提交前依据内部 provider catalog 校验开放字符串 Link。MessageStack 构造顺序为 system identity、UserInputs、SessionBackground、通用 Phase1 Background、WorkingContext、TurnTraceHeap、task prompt overlay；除 identity 使用 system role 外，其余用户态语境和 TaskPrompt 均使用 user role messages。ContextEngine 不向上层暴露 heap node 等可变状态持有者；语境变更只有作用域化信号的事务批次提交与 Turn 生命周期两类入口。TurnTraceHeap 在 Turn 内保存 canonical trace，通过 branch/leaf head、inspect 和带 continuation cursor 的有界 recall 渐进恢复；recall result 使用 foldable projection，后续压力回收折叠回 origin ref。TurnSummary 在 Turn 结束时提供输入、Working/Background 终态、trace digest、provider-neutral 完整 JSON trace 与 heap 元数据，由 TurnCompletion 交给 Session 持久化。
...
- App 模块是 TinySoul 的进程装配、生命周期、项目初始化和外部输入边界层。TinySoulAppBuilder 显式加载配置环境，构建 LLM、Workspace、Session、Agent Home、Memory、Action、Context、SignalBus、RuntimeTrap 与各级 loop runner，调用模块 registrar 装配 Workspace/Session/Home/Memory/core action executor，并按各 capability 的 effective settings 检查依赖、注册或移除 capability action；随后向 LLM action backend service 注入 workspace reference resolver 与 Agent Home HOW provider，再注册 Context pressure、Workspace Trash restore、Home runtime copy 等精确 Trap 处理器和未处理 RuntimeException fallback。外部输入源只生产 InputEvent，由 InputCommandParser 纯解析为输入意图；InputDispatcher 原子读取 TurnRunner 暴露的 active Turn scope，用同一快照分类输入并发出 `loop.control.request` / `context.input.append`，避免活跃状态与 scope 分离读取的竞态。CLI 保持无子命令即启动 App，并提供 `tinysoul init [DIRECTORY]` 把 package-owned 可编辑模板安装到不存在或空目录；它拒绝覆盖、没有 `--provider`，模板 provider 全部默认 disabled，用户只通过 TOML 和 `.env`/进程环境配置。wheel 必须包含只读 Action Catalog 与可编辑 configs/Home/project 模板；catalog 直接使用包内资源，configs/Home 由 init 复制后归项目所有，initializer 同时建立与 Home 平级的空顶层 `memory/`。AppBuilder 只做跨模块装配并按真实模块归属桥接配置错误，不直接实现业务能力或读取业务资源正文。Memory 通过独立 Engine、registrar、Background provider 和 Runtime bridge 装配，不再从 HomeEngine 取得服务。
