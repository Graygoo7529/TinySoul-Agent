...
- 完整工件 `workspace.write/rewrite` 与 `script.write/rewrite` 使用 180 秒 Catalog owner timeout，`LLMActionTaskRunner` 为协作收尾预留 5 秒；Context/Session 模型投影已隐藏内部 revision/digest，持久记录、validator、领域 Engine 与 Endpoint 保留完整事实。完成记录见 `docs/analysis/20260725-done-action authoring deadline and model context transparency execution plan.md`。
