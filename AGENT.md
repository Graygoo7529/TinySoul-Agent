...
`home.top.search` 只检索 WHAT、WHY、通用 HOW 与 MEMORY，不包含 `agent`、`how_domain` 或 `how_action`。搜索由 Home-owned 独立模块基于 effective catalog 构造有界 metadata，不物化全部 runtime copy，也不把完整正文自动加入 Background；标题取 Markdown 首个 H1，短摘要取首个有效正文段，缺失时分别回退 Link name 与有界正文前缀。搜索先以 link/name/title/summary/prefix 做确定性候选限制，候选上限为 20，默认 `top_k=5`、最大为 10，再通过受控 LLM task rerank；validator 只接受候选内 Link，rerank 失败时返回确定性候选而不使只读搜索整体失败。MEMORY 的搜索与读取仍只访问 actual Home，并使用稳定日期 Link。
...
当前进度：infra、runtime、llm、action、context、loop、app（含 TinySoulApp 装配入口、输入边界、ObservationEvent/OutputSink 三档输出和 console script）已完成；配置环境保留 main/include 来源诊断，各模块统一拒绝未知键；LLM provider 显式区分端点身份、adapter 和 enabled 状态，代理 provider 可独立持有凭据并提供模型。Session 已覆盖显式 business day、不可变 record、幂等 Turn 提交、确定性 summary、orphan reconciliation、归档前图恢复、只读 archive snapshot，以及为 Memory Maintenance 按需递归 Summary 图的专用 facts projection。Workspace 已覆盖 schema v3 day manifest、资源分类、digest 绑定描述、完整 reconciliation、write/rewrite/patch/delete、活动日 Trash 和 workspace/trash 日归档；一致性等级明确为单进程单写者、Engine 实例内线性化。Agent Home 已覆盖逐 Turn动态 Background、schema v2 跨日 overlay/schema v1 迁移、统一 effective top catalog/read、runtime-only top/tombstone、年月 MEMORY actual-read、渐进资源与 `home.top.*` mutation、Action Catalog 派生的 prompt mount write/patch、仅通用 HOW runtime 可用的 `SKILL_MEMORY.md`、无持久 review/apply 状态的 Home Maintenance、三段式 Memory Maintenance，以及 effective WHAT/WHY/HOW/MEMORY 的有界 metadata、确定性候选和 `home_search` LLM rerank/fallback；Stage 5 已提供各一个实际 WHAT/WHY/HOW 示例。三个 Home LLM task 分别使用 JSON-only `home_search`、`home_maintenance` 与 `memory_maintenance` profile。Stage 6 已把 Home/Memory Maintenance 接为 typed Program work：Program 启动与每项 work 先做 Daily preflight，启动提示 active Home diff 与昨日 Memory eligibility，命令在活跃 Turn 中仍进入 Program queue，App-owned decision broker 提供人工逐项确认，内置 scheduler 以业务时区午夜、`00:05`、`00:15` 依次投递 rollover、Home 和昨日 Memory；自动已有 MEMORY 跳过，manual 可重写，并发布三个最小 normal Program 事件。下一步在实施 Stage 7 前确认模块级 Observation 的精确 schema，以及 E2E 中“后续 Turn 可见 MEMORY”的具体入口，再补 daily/Home/Memory crash window 与无网络 E2E；之后推进 document conversion、受控 script action 和安装初始化闭环。每完成一个模块或重要切面，应同步补充「项目规约」中该模块的运行方式规约，并更新本节进度。
