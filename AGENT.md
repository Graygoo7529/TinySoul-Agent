...
- Endpoint 模块是本地桌面前端的外部协议适配层，只允许 loopback bind，并以进程级 bearer token 鉴权 HTTP、以首帧 token 鉴权 WebSocket。`tinysoul serve` 仍使用同一个 AppBuilder、ProgramRunner、InputDispatcher、DailyLifecycleCoordinator、SessionEngine、WorkspaceEngine 和 HomeDecisionBroker；Endpoint 不拥有业务状态、不直接访问持久目录。ObservationRouter 为每个 sink 保留独立 normal/verbose/model 上限，Endpoint event buffer 只做有界进程内 sequence replay/gap，不进入 Session 或控制流。Workspace/Session 请求持有 active-day lease；UI mutation 在同一 WorkspaceEngine 锁内校验 Manifest revision 与 resource digest，文本和 blob 都只通过 `workspace:` Link 访问，成功后在活跃 Turn 发布 full snapshot。Endpoint request failure 是稳定 HTTP 局部结果，服务启动失败经 RuntimeEndpointBridge 映射为 startup failure；FastAPI/Uvicorn/WebSockets 属于 `desktop` extra 并仅在 serve 路径延迟导入。
...
本轮修复了 provider 工具协议边界：Action Catalog 的 dotted action identity 保持不变，OpenAI SDK 形态适配器在单次请求内建立 provider-safe 名称映射，覆盖工具定义、assistant tool-call 历史、Chat tool-result name 和响应解码；provider-native 历史只回放已闭合的 call/result 对，工具禁用的 Phase3 嵌套 task 将 ToolResultMessage 转为普通上下文，不再发送当前尚无 ActionResult 的 Phase2 tool call。Kimi/GLM 只校验映射后无法消除的真实能力约束，不再把内部 action identity 或未完成框架 decision 误判为 provider 配置错误；Core Action deadline 调整为 60 秒以覆盖一次有限 provider 切换/重试窗口。
