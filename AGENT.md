...
- Action 局部失败的唯一事实源是 typed `ActionLocalFailure(reason, scope, disposition, feedback, constraint)`。success 禁止 failure，failed/timeout 必须携带 failure；业务 payload 不承载 `payload.failure`，frame data 不重复失败语义。普通 hook 拒绝通过 `HookOutcome.failure` 直接交付该事实，pipeline 只补 hook 诊断；hook 注册缺失或实现异常仍由 pipeline 产生自己的局部 failure。`ActionResultRenderer` 只是 Action-owned 投影边界，统一生成 model/trace 及 visible/canonical ToolResult envelope，不推断事实。foldable canonical result 保留 `action/status/stage/failure` 包络，只替换业务 `canonical_payload`。
...
- Session Background 是 Turn preparation 期间注入的固定、不可逐出历史段；超预算时以 immutable Summary 递归收缩，极端时使用 overflow head。`session.history.inspect` 无 ref 导航 authoritative root、以 Summary ref 展开 direct children、以 Turn ref 读取有界 preview；root cursor 绑定 Manifest revision。`session.history.actions` 只从具体 Turn 的 immutable canonical trace 确定性派生完整 Action summary、failure groups 与 trace indexes；projector 严格报告 missing/orphan/duplicate/name mismatch，name mismatch 拆为真实 call/result action occurrence。`session.history.recall` 只恢复具体 Turn canonical trace，不导航 Summary，不返回 Background/preview/Action summary。三个查询都是普通 Action，结果进入 Interaction 而不改写 SessionBackground；它们不调用 LLM，因此只维护 Catalog 和 Session Domain HOW，不建立 Action HOW。Endpoint 的 Session Explorer 使用 view-local ref/request identity 状态，actions/trace 独立收敛并保存有界页轨迹；切换 Turn/client 时取消旧请求，迟到响应仍由 reducer 拒绝，root revision 变化时清空整个旧导航快照。Turn record 直接使用当前 schema，不保留旧 schema adapter。
...
- Runtime feedback 优化已统一 Action typed failure/result rendering、canonical trace digest、Session Action history、Context/Session hard pager、Workspace edit read set 和四个 Working control identity；review follow-up 进一步补齐 char/entry 双硬上限、结构化 cursor failure、真实 action name mismatch 归属、Hook failure 单一来源以及 Endpoint/前端协议。对应真实运行问题与验收记录见 `docs/analysis/20260723-done-real runtime feedback optimization plan.md`。
- Session 历史边界已收敛为固定 Background、inspect 图导航、actions 确定性审计和 recall Turn trace 证据；Endpoint 分为 history/actions/trace 三条只读路由，前端 Session Explorer 不再消费模型侧 Background 状态。完成记录见 `docs/analysis/20260724-done-session history navigation boundary correction plan.md`。
