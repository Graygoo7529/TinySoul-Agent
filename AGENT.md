...
- Agent Home 模块负责 `home:` 链接、顶层背景目录、渐进式资源、domain/action HOW 和当日可写 overlay。原始内容严格位于 `home/` 且普通 User Turn 只读；运行时镜像严格位于 `runtime/home/`，由带 business day、revision、baseline/runtime digest、state 的 `HomeOverlayManifest` 和可前滚 operation journal 管理。非 MEMORY 内容在首次实际读取时通过 `HOME_RUNTIME_COPY_REQUIRED` Trap 透明物化，handler 只在磁盘确实由缺失变为存在时重试一次；历史 MEMORY 始终读取 original，不复制到 runtime。Context 每个 User Turn 清空 Home Background，再通过动态 `HomeBackgroundEntryProvider` 重建默认 core 和当前可加载目录，Phase1 临时加载项不跨 Turn 留存。`home.resource.read/write/patch/delete` 支持渐进资源的有界读取、runtime-only create、digest 前置条件、精确 patch 和 tombstone；顶层 WHAT/WHY/HOW/MEMORY 修改保留给后续 settlement/专用 action。日切只把完整 runtime Home 移入归档，不写回 original；top search、memory append、HOW 使用反馈和 settlement plan/review/apply 仍未实现。
