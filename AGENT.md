...
- Loop 模块是 TinySoul 的运行编排层，按 Program/Turn/Cycle/Phase/Module 消费 Runtime 运行位置与运行转移；Phase 单元只组合 ContextEngine、ActionEngine 与 LLMTaskRunner，不复制三者内部语义。TurnRunner 在首个 Cycle 前通过 `TurnPreparationPipeline` 执行可重放准备步骤：Session 先投影当日历史，Workspace 再投影 Manifest；指向当前 Turn 的 RETRY 只重放 preparation，不重复 begin Turn。`core.answer` 成功后由 Phase3 抛出 `runtime.turn_output`，TurnOutput Trap 校验结果、发出作用域化 `loop.turn.output` 并结束当前 Turn；多个成功 answer 记录为 `LoopTraceNoteKind.MULTIPLE_TURN_OUTPUTS`。Context 结束后，`TurnCompletionPipeline` 默认先由 Session 持久化完整 TurnSummary 与 TurnOutput，再运行其它后处理。Context pressure Trap 协调 trace/background/Workspace 回收；Workspace Trash restore Trap 恢复压力暂存资源、同步 Context 并重试当前 Module。处理器 RuntimeException 仍由原 Turn scope 的 Trap 收束，cycle 上限只作为兜底保护。
...
- Workspace 模块负责 `workspace:` 链接、`runtime/workspace` 当日工作区根目录、版本化 Manifest、磁盘 reconciliation、扫描诊断、临时 task prompt 输入、文件变更 action 和 PromptBlock 链接解析。磁盘是内容事实源，Manifest 是版本化索引与语义描述层，WorkingContext workspace 段是当前 Turn 的同 revision 投影。`WorkspaceReconciler` 独立承担发现、完整性判定和原子提交；`WorkspaceEngine` 串行化变更、description、Trash/restore 与 reconciliation。Manifest 只在完整 reconciliation 后提交；description 与当前 digest 绑定，内容变化后失效。资源稳定分为 text、image、document、binary：text 有界进入 Prompt，image 校验内容签名并受图片预算约束，document 必须经显式转换 action 生成可读资源，binary 只提供元数据。压力回收只移动未被当前 action target/reference links 保护的 ephemeral/turn 资源；批次中途失败必须回滚已移动项。Trash 位于 active root 外且不进入 Manifest，是可恢复暂存区：`workspace.trash.list` 暴露恢复引用，`workspace.restore` 支持显式恢复；只有 `context_pressure` 等框架暂存项在 active miss 时映射为 Trash restore Runtime 原因，由 Trap 恢复、同步 Context 并重试 Module，显式 `workspace.delete` 不自动撤销。Trash 前必须以当前磁盘 digest 为准，不能把过期 Manifest digest 绑定到新内容。WorkspaceEngine 不依赖 Context 类型，跨模块恢复协调归 Loop；Context 和 ActionResult 不保存文件正文。变更失败通常收敛为局部 ActionResult，RuntimeException 不被 Action runner 降级；workspace 配置错误由 workspace bridge 映射为模块归属的 startup failure。日终归档仍需继续在 Workspace 模块内扩展。
