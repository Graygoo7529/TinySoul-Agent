...
- Workspace 模块负责 `workspace:` 链接、当日工作区根目录、资源扫描、manifest 读写、单资源摘要刷新、扫描诊断、临时 task prompt 输入和 WorkingContext 资源摘要投影。`workspace.scan` 与 `workspace.describe` 由 Workspace 模块提供 handler/executor，通过 `context.working.patch` 信号同步 `WorkspaceResource(link, summary)`；Context 和 Action 结果不保存 workspace 文件正文。当前已实现扫描、单资源摘要刷新、扫描跳过诊断、Engine 内部有界文本前缀读取、`WorkspacePromptInput` 临时输入渲染与 manifest 切面；workspace 配置错误由 workspace bridge 映射为模块归属的 startup failure。复合 action 对临时输入的真实调用、write/patch/delete 和日终归档仍需继续在 Workspace 模块内扩展。
...
当前进度：infra、runtime、llm、action、context、loop、app（含 TinySoulApp 装配入口与输入边界）已完成；Workspace 与 Agent Home 已完成基础接入，覆盖 workspace 链接扫描、manifest、单资源摘要刷新、WorkingContext 摘要同步、扫描诊断、Engine 内部有界文本前缀读取、`WorkspacePromptInput` 临时输入渲染、home 顶层背景、how_action 注入、渐进式资源只读 action、runtime home 缺页式副本准备和模块配置错误归属。下一步建议继续推进 Workspace 复合 action 对临时输入的真实调用、write/patch/delete、Agent Home 检索与写入、HOW 使用反馈、memory append 和每日沉淀。每完成一个模块或重要切面，应同步补充「项目规约」中该模块的运行方式规约，并更新本节进度。
