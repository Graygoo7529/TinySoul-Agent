...
每日生命周期、确定性归档与 Settlement：Session、WorkSpace 和 Agent Home runtime 都具有明确的 Business Day。新日开始时，框架必须先完成不依赖 LLM 的确定性日切：恢复未完成的日切 journal，完整 reconcile 旧日 Session、Workspace、Home overlay，把旧日 active roots 和 Trash 冻结到同一个时间戳归档，再建立同一新日的空 active roots；在确定性日切完成前不能接受新日 User Turn。确定性归档只保存事实，不进行知识判断，也不修改原始 Agent Home。Settlement 是归档之后的独立语义维护任务：它消费不可变归档，分析 Session、Workspace、Home diff、记忆候选和 HOW 使用反馈，经过 plan/review/apply 决定如何更新原始 Home。Settlement 可以由夜间后台 Agent、程序启动时的 pending 检测或用户命令触发，不阻塞新日普通 User Turn；对 original Home 的 apply 必须使用稳定 operation id、digest 前置条件和可恢复 journal，不能把“归档完成”当成“沉淀完成”。

原始 Agent Home 在普通 User Turn 和确定性日切中严格只读。历史 MEMORY 始终只读 original，因为旧记忆不需要复制到 runtime 修改；其它 Home 顶层内容、渐进资源和 HOW 在真正使用时通过 Trap 透明复制到当日 runtime，并始终从 effective runtime overlay 读写。Phase1 或 action 只看到 `home:` Link，不感知 original/runtime 分层。对原始 Home 的写回只允许发生在 Settlement apply 或专用维护入口中，而不是由归档移动本身完成。对于 HOW 类型技能包，应形成当日可审计的使用反馈（例如 SKILL_MEMORY/DOMAIN_MEMORY 等等价事实），再由 Settlement 决定是否进入长期 Home。

目录边界如下：

- `home/`：长期原始内容；
- `runtime/`：仅保存当前 Business Day 的 active 状态；
  - `session/`：当日跨 Turn 会话事实；
  - `workspace/`：当日工作区及 active `.tinysoul/trash`；
  - `home/`：Agent Home 非 MEMORY 内容的按需镜像与当日 overlay；
- `archive/<timezone-timestamp>/`：一个已冻结旧日，包含 `transition.json`、`session/`、`workspace/`、`home/`、`trash/`；旧日 Trash 只是归档事实，不再进入 active list/restore 或其它语义追踪 API。
...
- Agent Home 模块负责 `home:` 链接、顶层背景目录、渐进式资源、domain/action HOW 和当日可写 overlay。原始内容严格位于 `home/` 且普通 User Turn 与确定性日切都只读；运行时镜像严格位于 `runtime/home/`，由带 business day、revision、baseline/runtime digest、state 的 `HomeOverlayManifest` 和可前滚 operation journal 管理。非 MEMORY 内容在首次实际读取时通过 `HOME_RUNTIME_COPY_REQUIRED` Trap 透明物化，handler 只在磁盘确实由缺失变为存在时重试一次；历史 MEMORY 始终读取 original，不复制到 runtime。Context 每个 User Turn 清空 Home Background，再通过动态 `HomeBackgroundEntryProvider` 重建默认 core 和当前可加载目录，Phase1 临时加载项不跨 Turn 留存。`home.resource.read/write/patch/delete` 支持渐进资源的有界读取、runtime-only create、digest 前置条件、精确 patch 和 tombstone；顶层 WHAT/WHY/HOW/MEMORY 修改保留给后续 settlement/专用 action。日切只把完整 runtime Home 移入归档，不写回 original；未来 Settlement apply 必须从不可变归档出发，以 baseline/current digest 校验、稳定 operation id 和 journal 更新 original，并与当日 active overlay 隔离。top search、memory append、HOW 使用反馈和 settlement plan/review/apply 仍未实现。
