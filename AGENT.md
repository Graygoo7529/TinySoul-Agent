...
- Context 的 `trace_summary` 是计数视图，`trace_digest` 是 canonical trace 稳定 JSON 的 SHA-256。Context/Session recall 共享 immutable JSON sequence hard pager，但分别拥有 ref/store 导航语义；字符预算与条目数量都使用 requested/effective 双层限制，`max_entries=1` 可精确读取已知 index；普通 entry 原子返回，oversized entry 只通过 digest-bound 服务端 cursor 按 Unicode 字符续读，最终 JSON 不得突破生效字符预算。只有 owner-defined typed request error 可转为局部 Action failure，I/O 与 invariant 经所属 Runtime bridge 结束 Turn。Working model tools 固定为 `set_milestone`、`remove_milestone`、`set_todo`、`remove_todo`，每次调用表达一个非空完整操作，多调用按 sequence 投影验证后原子消费；不保留 `update_working` alias。
- Session 只从 immutable canonical Turn trace 确定性派生 Action history。projector 严格扫描 Phase2 Action ToolCall 与 Phase3 ActionResult envelope，按 call id 报告 missing/orphan/duplicate/name mismatch，复用于 record、Background、inspect、reconcile 与 `session.history.actions`；name mismatch 拆为真实 call action 和 result action 两个异常 occurrence，避免把结果状态归错 action。inspect 的直接 Turn item携带同源 action outcome summary，summary item 不聚合子节点 Action counts。首个 recall page 仅在硬字符预算允许时携带派生 Background，超限时以 `background_state.reason=page_budget` 明确省略且 canonical trace 仍可分页恢复；不读取 capability 私有 payload，不把派生 summary 作为第二事实源。Turn record 直接使用当前 schema，不保留旧 schema adapter。
...
- Runtime feedback 优化已统一 Action typed failure/result rendering、canonical trace digest、Session Action history、Context/Session hard pager、Workspace edit read set 和四个 Working control identity；review follow-up 进一步补齐 char/entry 双硬上限、结构化 cursor failure、Session Background 明确省略、真实 action name mismatch 归属、Hook failure 单一来源以及 Endpoint/前端协议。对应真实运行问题与验收记录见 `docs/analysis/20260723 real runtime feedback optimization plan.md`。
