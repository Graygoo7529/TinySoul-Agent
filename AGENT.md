...
- `tinysoul/assets/project` 已成为唯一项目初始化模板源：公共 README、`.gitignore`、`tinysoul.toml` 与 Home 只维护一份，standard/development 只分别保存完整 `configs/` 与 `.env.example`。`tinysoul init` 默认物化安全 standard，也可显式选择不含凭据的 development；源码仓库根不再作为可运行项目或真实 provider 配置源。
