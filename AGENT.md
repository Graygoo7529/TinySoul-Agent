...
（1）AGENT.md 是顶层内容，类似一本参考全书目录，记录 AGENT 所具有的整体规范、行为设定、核心规则、用户偏好等。运行时 core 使用 `home:agent@AGENT.md`，其它 Agent 顶层内容使用相对于 `home/agent/` 的真实 Markdown 文件路径，例如 `home:agent@user/user.md`；在 user.md 中可以进一步链接渐进式内容。`home:*@...` 只表示该 Link 是可进入 BackgroundContext 的顶层内容，不等于自动加载；Home provider 每个 User Turn 必须自动加载不可逐出的 core，并在 effective `home:agent@user/user.md` 存在时同时自动加载其不可逐出的正文，其它 Agent/WHAT/WHY/通用 HOW 顶层内容按需加载；
...
（3）WHY 是另一个 Knowledge 库，用于标注问题的原因和解答。WHY Link 使用相对于 `home/why/` 的真实 Markdown 文件路径，文件名应直接表达问题，例如 `home:why@why-is-updating-home-important.md`；WHY 文档是顶层内容，并可通过语义匹配 top-k 交付语境模块；
