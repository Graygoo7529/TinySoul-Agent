...
- Session Background 是 Turn preparation 期间注入的固定、不可逐出历史段；超预算时以 immutable Summary 递归收缩，极端时使用 overflow head。自动 Turn Background 只提供 kind/ref、ask、answer、references、exhausted 与按 Action 名称聚合的 success/failed/timeout；仅在 canonical trace 存在配对异常时为对应 Action 增加粗粒度 `incomplete_count`，不暴露 trace summary、完整 pairing 审计、occurrence detail、digest/revision 或任意 ActionResult 业务 payload。持久 Turn preview、validator、Endpoint 与显式 Session 查询仍保留各自需要的完整事实和配置选中的有界 status/failure detail。`session.history.inspect` 无 ref 导航 authoritative root、以 Summary ref 展开 direct children、以 Turn ref 读取有界 preview，且不扫描 Action；root cursor 绑定 Manifest revision。`session.history.actions` 只从具体 Turn 的 immutable canonical trace 确定性派生完整 Action summary、failure groups 与 trace indexes；projector 严格报告 missing/orphan/duplicate/name mismatch，name mismatch 拆为真实 call/result action occurrence。`session.history.recall` 只恢复具体 Turn canonical trace，不导航 Summary，不返回 Background/preview/Action summary。Session-owned Turn/Summary validators 在写入、生命周期 reconciliation、inspect/actions/recall、自动 Context 投影与 Memory projection 统一校验 record 结构、确定性身份及物化 Background；Turn 的 ask/answer/references/outcome/trace identity 从内在事实重投影，历史 action detail 不按当前配置重选，但必须逐项证明来自 canonical trace。canonical trace 是证据，digest 是完整性事实，action history 和 Background 都不是第二事实源。三个查询都是普通 Action，结果进入 Interaction 而不改写 SessionBackground；Agent Action adapter 移除 source/summary/preview 的 trace digest 与 source revision，cursor binding 保持 opaque 并只需原样回传。SessionEngine 与 Endpoint 领域查询仍保留完整事实。无 ref inspect 只观察最近一次已提交的 authoritative Manifest root，已知 ref 可读取原子落盘且有效但尚未接入 root 的 orphan record，二者都不触发 reconciliation 或 revision 变化。Endpoint 的 Session Explorer 必须以 root revision/status/event 判断已提交导航头部；view-local actions/trace 独立收敛并保存有界页轨迹，切换 Turn/client 时取消旧请求，迟到响应仍由 reducer 拒绝，root revision 变化时清空整个旧导航快照。三个查询都不调用 LLM，因此只维护 Catalog 和 Session Domain HOW，不建立 Action HOW。Turn record 直接使用当前 schema，不保留旧 schema adapter。
...
- 完整工件 `workspace.write/rewrite` 与 `script.write/rewrite` 使用 180 秒 Catalog owner timeout，`LLMActionTaskRunner` 为协作收尾预留 5 秒并在成功返回边界重检；Context 已删除无消费者的 Trace anchor/canonical revision。Session 自动 Background 已收敛为 ask/answer/reference 与逐 Action 聚合 outcome，不再显示 trace/pairing/occurrence 内部结构；真实 cursor、持久记录、validator、显式领域查询与 Endpoint 仍保留各自需要的完整事实。完成记录见 `docs/analysis/20260725-done-action authoring deadline and model context transparency execution plan.md`。
