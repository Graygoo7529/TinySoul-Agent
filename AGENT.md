...
Maintenance 是与 User Turn 同级的 Program work。`tinysoul.app` 拥有顶层请求队列和 Program frame，并将 `UserTurnRequest` 分派给 User Turn，将 `MaintenanceRequest` 分派给 `MaintenanceEngine`，将 `ExitRequest` 交给 Program trap；输入源和 scheduler 只能投递 typed request，不能越过 Program 直接操作 Session、Workspace、Home 或 Memory。手动命令与定时触发只在 request 的 trigger、目标日期和可选 rebuild 参数上不同，进入 Maintenance 后使用同一个 plan、任务编排和结果协议；全程自动执行，不需要人工 approval，不建立 decision channel，也不因等待人类输入而阻塞。显式命令为 `/maintenance` 或 `/maintenance daily`、`/maintenance home`、`/maintenance memory [YYYY-MM-DD] [--rebuild]`。长运行 App 的 scheduler 按 `maintenance.schedule` 每日投递一个 Daily request；程序启动只补做确定性日切并报告未完成的 Home/Memory work，不追补启动前错过的 LLM Maintenance。计划时刻前已启动的进程会按时投递；运行中跨过计划时刻或多日休眠后只合并投递一个当前日 Daily request。

`tinysoul.maintenance` 是唯一维护门面和业务 owner，并分为 Archive、Home、Memory 三个独立包。Archive 先执行不依赖模型的日切、恢复和只读归档投影，禁止把仍开放的当天作为 Memory 目标；Daily request 随后处理 Home，并处理 archive catalog 中所有仍符合条件的已关闭日期 Memory，因此停机期间漏掉的工作不会依赖一个临时“昨日”猜测。Home Maintenance 逐个 inspect runtime Home diff，并通过专用 action 接受、拒绝或整理改写后合并到 actual Home；完成条件由 Home owner 校验所有 diff 与 `SKILL_MEMORY.md` 均已处理，随后移除空的 `runtime/home`，下次使用时再懒加载。它不创建 Home archive，也不持久化平行的 plan、review result 或 approval 状态；中断后仍以尚存 diff 为事实重新开始。Memory Maintenance 只读取指定日期的 Archive projection，包括 Session facts、Session 背景与 Workspace manifest，并通过专用 action 调用 Memory owner 完整写入日期 MEMORY；默认不覆盖已存在的有效 MEMORY，显式 `--rebuild` 才把旧 MEMORY 作为同日 consolidation source。Home 或某个日期 Memory 失败只形成各自 task outcome，不回滚已完成的其它维护任务。

User Turn 与 Maintenance Turn 复用 `tinysoul.loop` 的 Turn、Cycle 和相同 3-stage Phase 内核，但由 `loop.user` 与 `loop.maintenance` 分别提供 preparation、completion、prompt guidance 和 outcome 语义。Maintenance Turn 仍处于完整情景中：它拥有与 User Turn 同结构的 Background、Session 和 Workspace；Home task 使用 actual Home 作为 Background 基线并结合当前 Session/Workspace，Memory task 使用 actual Home 与目标关闭日的 Session/Workspace 只读投影。区别只在 task input、维护专用指引、完成协议和可见 Action Catalog，而不是另造一个脱离 Context 的模型调用。通用只读 `core.context.inspect` 与 `core.session.inspect` 可由两类 Turn 复用；`maintenance.*` actions 只装配到对应 Home 或 Memory Maintenance Turn，User Turn 的 ActionEngine 明确排除整个 maintenance domain，Home 与 Memory 两种 Maintenance Turn 也互不暴露对方的 actions。Maintenance Turn 不提供 `core.answer`，必须由 owner-bound `maintenance.complete` 验证任务后置条件后结束。
...
- Loop 模块只拥有可复用的 Turn/Cycle/Phase 运行内核，不拥有 Program、请求队列、scheduler 或每日生命周期。`turn.py` 负责通用 Turn runner，`cycle.py`/`phases.py` 负责共用 3-stage 内核；`loop.user` 和 `loop.maintenance` 分别组合 preparation、completion detector、task guidance 与 outcome 语义。Phase 只组合 ContextEngine、ActionEngine 与 LLMTaskRunner，不复制三者内部语义。两类 Turn 都在开始时捕获一个权威 `BusinessDay` 并在该 Turn 内保持不变，也都按 Background、Session、Workspace 的顺序构造 Context；指向当前 Turn 的 RETRY 只重放 preparation，不重复 begin Turn。User Turn 由 `core.answer` 经 TurnOutput Trap 结束，并由 Session completion 持久化 sealed schema v4 Turn；Maintenance Turn 不产生用户回答或写入 User Session，只由 `maintenance.complete` 和任务 owner 的后置条件结束。`TurnOutcomeStatus`/`TurnFailure` 对两类 Turn 使用同一结构化运行结果。
- Maintenance 模块拥有业务时钟、确定性日切、archive catalog、到期计算、计划和 Archive/Home/Memory 任务编排。Archive 以 `.pending-*` journal 按 Session、Workspace/Trash 顺序归档旧日，在旧日 roots 移出 active runtime 后初始化同一新日 roots，最终形成不含 Home 的 `archive/<timestamp>/`；进程级中断通过 persisted facts 幂等前滚。Home 不参与 active day claim、daily archive 或新日初始化。Archive 只向其它维护任务提供按 Business Day 定位的只读 Session/Workspace roots；Session 与 Workspace 各自校验和解释内部事实，Maintenance 不读取它们的私有 store。Home/Memory 需要模型推理时调用独立 Maintenance Turn，不把推理塞进 Archive 流程。
- App 模块是 TinySoul 的进程装配、顶层 Program、项目初始化和外部输入边界层。`app.program` 拥有串行请求队列与 Program frame，只分派 `UserTurnRequest`、`MaintenanceRequest` 和 `ExitRequest`；`app.sources` 中的 Terminal、Endpoint adapter 与 scheduler 都只进入这一边界。外部 `InputEvent` 由 InputCommandParser 纯解析，InputDispatcher 基于一次 active Turn scope 快照将普通输入变为新 User Turn 或当前 Turn append/control，并始终将 Maintenance 命令变为 Program queue 中的 MaintenanceRequest。TinySoulAppBuilder 显式加载配置环境，构建 LLM、Workspace、Session、Home、Memory、Action、Context、SignalBus、RuntimeTrap、User/Maintenance Turn 与 MaintenanceEngine，并为 User、Home Maintenance、Memory Maintenance 分别构建精确 ActionEngine view。`tinysoul start` 是唯一交互运行入口，同一 App 同时拥有 Terminal 与 Endpoint；`--mode` 只控制 Console route，Endpoint route 固定为 model。每个项目启动前持有 App-owned 进程 lease，Endpoint ready 后发布临时连接描述，退出时清理；重复启动不得构建第二个业务 Engine。`tinysoul start --once` 保留无交互单 User Turn 验收且同样持有 lease。`tinysoul/assets/project` 是唯一 init/reset 模板源；AppCommandGateway 只统一可信命令输入，输出统一经 ObservationEmitter/Router 分发。AppBuilder 只做跨模块装配并按真实模块归属桥接配置错误，不直接实现业务能力或读取业务资源正文。
...
- Agent Home 模块负责 `home:` 链接、顶层背景目录、渐进式资源、domain/action HOW、通用 HOW 的 `SKILL_MEMORY.md` 和跨日可写 overlay。actual 内容严格位于 `home/` 且普通 User Turn 与确定性日切都只读；runtime overlay 严格位于 `runtime/home/`，由 revision、baseline/runtime digest、state 和可前滚 operation journal 管理，但不再以 Business Day 作为身份或归档边界。Home 内容在首次实际读取时通过 `HOME_RUNTIME_COPY_REQUIRED` Trap 透明物化，handler 只在调用前 runtime 文件缺失且调用后确实存在时重试一次。User Turn 的 Context 使用 effective Home；Maintenance Turn 使用 `ActualHomeBackgroundEntryProvider`，保证维护判断以已提交 actual Home 为 Background 基线。通用 `SKILL.md` frontmatter、Top/Resource/Prompt Mount Link 和搜索规则保持由 Home owner 统一校验，所有普通修改只落 runtime。`HomeMaintenanceService` 只从 active overlay 与 current actual 构造有界 change snapshot，并按 Maintenance action 明确给出的 accept/reject/rewrite resolution 执行单项 owner mutation；它不拥有 LLM reviewer、人工 decision provider、stdin 或审批状态。copied 与已和 actual 一致的 record 确定性清理，core tombstone 在处理前拒绝；accept/rewrite 使用单文件原子替换，reject 保留 actual，三者完成后都移除对应 overlay record/content。`SKILL_MEMORY.md` 作为同 skill change 的维护上下文并在该 skill 处理完成后清理。全部 pending 解决后 `finalize_maintenance` 删除空 runtime Home；actual 写后清理前中断时，下次通过 runtime/actual 一致性自动收束。Home 不解释 `memory:` Link、Memory 目录、Memory search/recall 或 Memory Maintenance 编排。
