...
每个 User Turn 的 preparation 都按同一 Business Day 与业务时区确定“昨日”；若精确的昨日 MEMORY 存在，Memory provider 将完整但受文档上限约束的正文作为自动 Background entry 交给 Context。缺失昨日 MEMORY 是正常状态，不回退到更早日期；文件存在但无法解释时是 Memory 模块失败。该 entry 每 Turn 重建，在 Context 压力回收时可被逐出；Home core 仍是不可逐出的默认规约。其它日期 MEMORY 不进入可加载 Home 目录，只能通过 `memory.search(query, top_k)` 与 `memory.recall(memory_link)` 按需访问；search 返回稳定 Link、日期、最相关时间段和有界摘要，recall 返回完整但受上限约束的单日 Markdown，两者的 ActionResult 都进入当前 TurnTraceContext，不修改 Background。

Memory Maintenance 读取 Session 为指定日期提供的专用 facts projection：Session 按需递归已提交 Summary 图，只交付唯一、可达的 Turn 事实，不暴露 store 或 archive 文件结构。Turn 按业务时区中的开始时间归入上午 `[00:00, 12:00)`、下午 `[12:00, 18:00)`、晚上 `[18:00, 24:00)`；facts 与可选同日期旧 MEMORY 在各时间段内执行有界分层 consolidation，不静默截断。LLM 严格返回三个时间段的 Markdown body，Memory 确定性渲染日期和中文段标题。正文中的 `<home:space@name>` 必须指向当前 actual Home 中已存在的顶层内容，`<memory:YYYY-MM-DD>` 必须指向已存在的其它日期 MEMORY；非法或不存在的 Link 以有界模型反馈重新生成。Session archive 缺失或 projection 为空时 `skipped` 且不创建、不覆盖、不删除 MEMORY；否则生成完整新 MEMORY 并原子覆盖，不 append、不读取其它日期 MEMORY 正文作为 consolidation 输入。启动自动提示只检查昨日且要求 projection 非空：存在昨日非空 Session facts 且不存在昨日 MEMORY 时提示；不自动扫描更早日期，人工命令仍可显式指定日期。
...
- Context 模块向上层提供唯一装配与调用入口（组装门面），持有本轮 UserInputs、Context-owned Background、WorkingContext 与 TurnTraceHeap，负责构造式 MessageStack、语境控制工具与压力回收服务。Background 通过通用 provider 协议聚合 Home 顶层条目、昨日 Memory 与 Session 历史，不归 Home 或 Memory 任一模块所有。MessageStack 构造顺序为 system identity、UserInputs、SessionBackground、通用 Phase1 Background、WorkingContext、TurnTraceHeap、task prompt overlay；除 identity 使用 system role 外，其余用户态语境和 TaskPrompt 均使用 user role messages。ContextEngine 不向上层暴露 heap node 等可变状态持有者；语境变更只有作用域化信号的事务批次提交与 Turn 生命周期两类入口。TurnTraceHeap 在 Turn 内保存 canonical trace，通过 branch/leaf head、inspect 和带 continuation cursor 的有界 recall 渐进恢复；recall result 使用 foldable projection，后续压力回收折叠回 origin ref。TurnSummary 在 Turn 结束时提供输入、Working/Background 终态、trace digest、provider-neutral 完整 JSON trace 与 heap 元数据，由 TurnCompletion 交给 Session 持久化。
...
当前进度：infra、runtime、llm、action、context、loop、app（含 TinySoulApp 装配入口、输入边界、ObservationEvent/OutputSink 三档输出和 console script）已完成；配置环境保留 main/include 来源诊断，各模块统一拒绝未知键；LLM provider 显式区分端点身份、adapter 和 enabled 状态，代理 provider 可独立持有凭据并提供模型。Session 已覆盖显式 business day、不可变 record、幂等 Turn 提交、确定性 summary、orphan reconciliation、归档前图恢复、只读 archive snapshot，以及为 Memory Maintenance 按需递归 Summary 图的专用 facts projection。Workspace 已覆盖 schema v3 day manifest、资源分类、digest 绑定描述、完整 reconciliation、write/rewrite/patch/delete、活动日 Trash 和 workspace/trash 日归档；一致性等级明确为单进程单写者、Engine 实例内线性化。Agent Home 已覆盖逐 Turn 动态 Background、schema v2 跨日 overlay/schema v1 迁移、统一 effective top catalog/read、runtime-only top/tombstone、渐进资源与 `home.top.*` mutation、Action Catalog 派生的 prompt mount write/patch、仅通用 HOW runtime 可用的 `SKILL_MEMORY.md`、无持久 review/apply 状态的 Home Maintenance，以及 effective WHAT/WHY/HOW/MEMORY 的旧有有界搜索。Stage 4 至 Stage 6 已在 `tinysoul.home` 内实现三段式 Memory Maintenance、旧 `home/memory` 读取与 typed Program work：Program 启动与每项 work 先做 Daily preflight，启动提示 active Home diff 与昨日 Memory eligibility，命令在活跃 Turn 中仍进入 Program queue，App-owned decision broker 提供人工逐项确认，内置 scheduler 以业务时区午夜、`00:05`、`00:15` 依次投递 rollover、Home 和昨日 Memory。已确认的 Stage 6.1 尚未实施：需要把旧 Memory 实现从 Home 拆为 `tinysoul.memory`，切换到顶层 `memory/` 与 `[memory]` 配置，建立 `memory:YYYY-MM-DD`、`memory.search`、`memory.recall`，并将 Home Background 提升为 Context-owned 多 provider Background，每 Turn 自动加载可逐出的昨日 MEMORY。Stage 7 将在该拆分后补 daily/Home/Memory crash window 与无网络 E2E；“后续 Turn 可见 MEMORY”已明确为昨日自动 Background 与其它日期 search/recall 两个入口，模块级 Observation schema 仍需在 Stage 7 实施前确认。每完成一个模块或重要切面，应同步补充「项目规约」中该模块的运行方式规约，并更新本节进度。
