...
- Action 局部失败的唯一事实源是 typed `ActionLocalFailure(reason, scope, disposition, feedback, constraint)`。success 禁止 failure，failed/timeout 必须携带 failure；业务 payload 不承载 `payload.failure`，frame data 不重复失败语义。`ActionResultRenderer` 只是 Action-owned 投影边界，统一生成 model/trace 及 visible/canonical ToolResult envelope，不推断事实。foldable canonical result 保留 `action/status/stage/failure` 包络，只替换业务 `canonical_payload`。
- Context 的 `trace_summary` 是计数视图，`trace_digest` 是 canonical trace 稳定 JSON 的 SHA-256。Context/Session recall 共享 immutable JSON sequence hard pager，但分别拥有 ref/store 导航语义；普通 entry 原子返回，oversized entry 只通过 digest-bound 服务端 cursor 按 Unicode 字符续读，最终 JSON 不得突破生效字符预算。Working model tools 固定为 `set_milestone`、`remove_milestone`、`set_todo`、`remove_todo`，每次调用表达一个非空完整操作，多调用按 sequence 投影验证后原子消费；不保留 `update_working` alias。
- Session 只从 immutable canonical Turn trace 确定性派生 Action history。projector 严格扫描 Phase2 Action ToolCall 与 Phase3 ActionResult envelope，按 call id 报告 missing/orphan/duplicate/name mismatch，复用于 record、Background、inspect、reconcile 与 `session.history.actions`；不读取 capability 私有 payload，不把派生 summary 作为第二事实源。Turn record 直接使用当前 schema，不保留旧 schema adapter。
...
- Runtime feedback 优化已统一 Action typed failure/result rendering、canonical trace digest、Session Action history、Context/Session hard pager、Workspace edit read set 和四个 Working control identity；对应真实运行问题与验收记录见 `docs/analysis/20260723 real runtime feedback optimization plan.md`。
