...
- Session Background 是 Turn preparation 期间注入的固定、不可逐出历史段；超预算时以 immutable Summary 递归收缩，极端时使用 overflow head。自动 Turn Background 只提供 kind/ref、ask、answer，以及存在时的 references、exhausted 和 `actions`；`actions` 以一个 `session:turn/<turn_id>#actions` 虚拟集合 ref 绑定按 Action 名称聚合且只保留非零项的 success/failed/timeout/incomplete counts，不暴露 trace summary、完整 pairing 审计、occurrence detail、digest/revision 或任意 ActionResult 业务 payload。虚拟集合 ref 与 `session:turn/<turn_id>#action/<occurrence>` leaf ref 都由已校验 immutable Turn 的确定性 projector 派生，不落盘、不进入 Manifest，也不是新的 SessionRecord 或第二事实源；模型只复制 ref，不解释 occurrence 后缀。`session.history.inspect` 无 ref 导航 authoritative root、以 Summary ref 展开 direct children、以 Turn ref 读取简洁 overview、以 Action 集合 ref 分页列出 compact occurrence leaf，并可按已知 Action 名称过滤；root cursor 绑定 Manifest revision，其余 cursor binding 保持 owner-opaque。`session.history.recall` 只接受 inspect 签发的 Action leaf ref，返回该 occurrence 的语义化 request、outcome、result/references 或 failure，不向模型交付 entry index、call/result trace index、call id、cycle id、历史 stage 或完整 canonical trace。两项查询都是普通 foldable Action，完整结果进入当前 Interaction，回收后只保留 origin ref/continuation，不改写固定 SessionBackground；当前 Turn 的冷轨迹仍只由 Context trace inspect/recall 恢复。Session-owned projector 继续在写入、生命周期 reconciliation、显式查询、自动 Context 投影、Memory projection 和 Endpoint 查询中统一校验 record、call/result 配对与派生事实，严格报告 missing/orphan/duplicate/name mismatch；canonical trace 是证据，digest 是完整性事实，持久 action history、Background 和虚拟 ref 都不是独立事实源。Endpoint 的 history/actions/trace 领域查询继续保留 revision、完整 Action summary、failure groups、trace indexes 与 canonical trace，供前端诊断视图使用，不经 ActionEngine，也不得回灌模型 Background。普通查询不触发 orphan reconciliation 或 revision 变化；已知持久 Turn/Summary ref 仍可读取原子落盘且有效但尚未接入 root 的 orphan record。两个模型查询都不调用 LLM，因此只维护 Catalog 和 Session Domain HOW，不建立 Action HOW。Turn record 直接使用当前 schema，不保留旧 schema adapter。
...
- 完整工件 `workspace.write/rewrite` 与 `script.write/rewrite` 使用 180 秒 Catalog owner timeout，`LLMActionTaskRunner` 为协作收尾预留 5 秒并在成功返回边界重检；Context 已删除无消费者的 Trace anchor/canonical revision。Session 自动 Background 已收敛为 ask/answer/reference 与带虚拟集合 ref 的逐 Action 聚合 outcome，模型通过 inspect/recall 渐进展开语义化 Action request/result，不再接收 trace/pairing/index/execution 内部结构；持久记录、validator 和 Endpoint history/actions/trace 仍保留各自需要的完整事实。完成记录见 `docs/analysis/20260725-done-action authoring deadline and model context transparency execution plan.md` 与 `docs/analysis/20260725-done-session progressive action recall plan.md`。
