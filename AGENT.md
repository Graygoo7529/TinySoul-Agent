...
- 完整工件 `workspace.write/rewrite` 与 `script.write/rewrite` 使用 180 秒 Catalog owner timeout，`LLMActionTaskRunner` 为协作收尾预留 5 秒并在成功返回边界重检；Context 已删除无消费者的 Trace anchor/canonical revision，Session 模型投影隐藏内部 revision/digest，真实 cursor、持久记录、validator、领域 Engine 与 Endpoint 仍保留各自需要的完整性事实。完成记录见 `docs/analysis/20260725-done-action authoring deadline and model context transparency execution plan.md`。
