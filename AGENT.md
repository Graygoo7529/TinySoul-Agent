...
- Script capability 只通过 `script.write/rewrite/patch/promote` 维护脚本资源，通过分离的 `script.run_python`/`script.run_bash` 和 `wait/stop/read_candidate/apply/discard` 执行与监督，不暴露任意 inline command。临时脚本严格位于 `workspace:scripts/...`；长期脚本严格位于 `home:how/<existing-skill>/scripts/...`，由显式 promote 写入 lazy runtime Home 后等待 Home Maintenance，不隐式创建 HOW。每次运行使用 active Workspace 的有界事务 mirror、最小环境、`shell=False`、source policy、deadline/日志/产物配额和进程树终止；这是事务隔离加策略检查，不宣称 OS 硬沙箱。成功进程也只进入 `ready_to_apply`，必须显式 apply/discard；failed/timed_out/stopped 只能 inspect/read/discard。每 Turn 最多一个 unresolved job，job 不持久化、不跨 Turn；`core.answer` 在 job 未收尾时局部拒绝。普通 Cycle 预算耗尽后，Turn activity controller 只为有效 job 提供有界额外 Cycle，每个 Cycle 仍从 Phase1 开始，job ActionResult 只进入 TurnTrace interaction context；Turn stop/failed/exhausted/transfer/离开时终止并 best-effort 清理。apply 按启动 baseline 做逐路径冲突检查，不同路径并发修改可保留，同路径冲突拒绝整批提交并保留 job。
...
能力扩展 Stage 3 已完成：Script authoring/promote、Python 默认/Bash opt-in、事务 Workspace mirror、显式 apply/discard、Turn-scoped 监督 job、额外 Cycle pacing、answer guard、结束清理、全量回归和 wheel 发布验证均已闭环；后续继续按能力扩展执行计划进入尚未完成的 Stage，不扩大为任意宿主命令执行。
