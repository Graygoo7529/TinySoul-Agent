...
from tinysoul.plugins.capabilities.shell.config import (
    ShellAdapterSettings,
    ShellSettings,
)
from tinysoul.plugins.capabilities.shell.dependencies import shell_dependency_requirements
from tinysoul.plugins.capabilities.shell.errors import ShellContractError
from tinysoul.plugins.capabilities.shell.models import ShellInterpreter
from tinysoul.plugins.capabilities.shell.policy import ShellPolicy
from tinysoul.plugins.capabilities.shell.process import (
...
from tinysoul.kernel.context.trace import TraceKind
...
from tinysoul.kernel.loop import Phase3Unit
...
async def test_powershell_success_without_diff_completes_and_cleans(local_tmp: Path) -> None:
    workspace = _workspace(local_tmp)
    manager = _manager(local_tmp, workspace)

    observation = await _start_shell(
        manager,
        turn_id="turn_1",
...
@pytest.mark.skipif(shutil.which("powershell") is None, reason="PowerShell unavailable")
async def test_powershell_workspace_diff_requires_apply(local_tmp: Path) -> None:
    workspace = _workspace(local_tmp)
    manager = _manager(local_tmp, workspace)
    command = (
...
@pytest.mark.skipif(shutil.which("powershell") is None, reason="PowerShell unavailable")
async def test_failed_shell_job_is_retained_and_cannot_apply(local_tmp: Path) -> None:
    workspace = _workspace(local_tmp)
    manager = _manager(local_tmp, workspace)
    observation = await _start_shell(
        manager,
...
async def test_cmd_uses_the_same_supervised_lifecycle(local_tmp: Path) -> None:
    workspace = _workspace(local_tmp)
    manager = _manager(local_tmp, workspace)
    observation = await _start_shell(
        manager,
...
async def test_bash_opt_in_uses_the_same_supervised_lifecycle(local_tmp: Path) -> None:
    workspace = _workspace(local_tmp)
    manager = _manager(local_tmp, workspace)
    observation = await _start_shell(
        manager,
...
async def test_shell_timeout_and_stop_remain_owned_until_discard(local_tmp: Path) -> None:
    workspace = _workspace(local_tmp)
    ...
    timed_out = await timeout_manager.start(
    ...
    with pytest.raises(SupervisedProcessStateError):
        await timeout_manager.apply(
            turn_id="turn_timeout",
            execution_id=timeout_id,
        )
    await timeout_manager.discard(
        turn_id="turn_timeout",
        execution_id=timeout_id,
    )

    stop_manager = _manager(local_tmp / "stop", workspace)
    running = await stop_manager.start(
        turn_id="turn_stop",
        owner=SupervisedProcessOwner.SHELL,
...
async def test_failed_shell_candidate_can_be_read_then_discarded(local_tmp: Path) -> None:
    workspace = _workspace(local_tmp)
    manager = _manager(local_tmp, workspace)
...
@pytest.mark.skipif(shutil.which("powershell") is None, reason="PowerShell unavailable")
async def test_script_and_shell_share_one_job_resolved_by_execution_id(
    local_tmp: Path,
) -> None:
    workspace = _workspace(local_tmp)
    manager = _manager(local_tmp, workspace)
    running = await _start_shell(
        manager,
        turn_id="turn_1",
        interpreter=ShellInterpreter.POWERSHELL,
        executable=shutil.which("powershell") or "powershell",
...
async def test_shared_answer_guard_rejects_shell_owned_unresolved_job(
    local_tmp: Path,
) -> None:
    workspace = _workspace(local_tmp)
    manager = _manager(local_tmp, workspace)
