...
from tinysoul.plugins.capabilities.web.dependencies import web_dependency_requirements
from tinysoul.plugins.capabilities.web.backends.discovery import (
    DiscoveryRequest,
    discover_pages,
)
from tinysoul.plugins.capabilities.web.backends.network import FetchedPage
...
async def test_crawlee_discovery_returns_direct_candidates_and_recursive_metadata() -> (
    None
):
    pages = {
...
async def test_discovery_service_returns_complete_inline_result(
    local_tmp: Path,
) -> None:
...
def _discovery_execution() -> ActionExecution:
    with builtin_action_catalog_root() as root:
        action = ActionCatalogLoader().load(root).get_action(WEB_DISCOVER_PAGES_ACTION)
