...
async def test_home_top_search_uses_effective_metadata_without_materializing_actual(
    ...
    _write(
        ...
        _skill(
            "Daily Lifecycle",
            "Deterministic rollover and reflection boundaries.",
        ),
    )
    _write(
...
async def test_home_top_search_validates_rerank_and_falls_back_deterministically() -> (
    None
):
    ...
    valid = _StubReranker(("home:skills@beta", "home:skills@alpha"))
...
async def test_home_top_search_action_uses_llm_profile_and_returns_metadata(
    ...
    assert not (tmp_path / "runtime" / "home" / "skills" / "beta" / "SKILL.md").exists()


async def test_home_top_search_reads_applied_actual_after_home_reflection(
    tmp_path: Path,
) -> None:
