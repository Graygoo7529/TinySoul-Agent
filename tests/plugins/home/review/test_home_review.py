...
def test_home_reflection_resolves_accept_reject_and_rewrite(tmp_path: Path) -> None:
...
def test_home_reflection_rejects_stale_change_token(tmp_path: Path) -> None:
    actual = tmp_path / "home" / "agent" / "review.md"
    actual.parent.mkdir(parents=True)
    actual.write_text("old", encoding="utf-8")
    home = _home(tmp_path)
...
def test_home_reflection_finalize_requires_all_diffs_and_removes_runtime(
    tmp_path: Path,
) -> None:
    home = _home(tmp_path)
...
def test_home_reflection_next_access_recreates_runtime_overlay(tmp_path: Path) -> None:
    home = _home(tmp_path)
    assert home.remove_resolved_overlay() is True
    assert not home.runtime_root.exists()
...
def test_home_reflection_does_not_remove_unowned_runtime_metadata(
    tmp_path: Path,
) -> None:
    home = _home(tmp_path)
...
def test_home_reflection_restores_runtime_root_when_final_delete_fails(
    tmp_path: Path,
    ...
    monkeypatch.setattr(
        "tinysoul.plugins.home.overlay.manager.shutil.rmtree", fail_remove
    )
