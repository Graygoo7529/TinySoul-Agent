...
from tinysoul.plugins.home.overlay import HomeOverlayManager
...
def test_home_overlay_mutations_survive_restart_without_touching_original(
    ...
    manifest = json.loads(
        (tmp_path / "runtime" / "home" / ".tinysoul" / "home_overlay.json").read_text(
            encoding="utf-8"
        )
    )
...
def test_prompt_mounts_follow_action_catalog_and_mutate_only_runtime(
    ...
    assert home.guidance_for_action("workspace", "workspace.compose") == (
...
def test_runtime_home_rejects_memory_overlay_content(tmp_path: Path) -> None:
    ...
    original.mkdir()
    invalid = tmp_path / "runtime" / "home" / "memory" / "2026" / "07" / "2026-07-11.md"
    invalid.parent.mkdir(parents=True)
...
def test_home_builder_migrates_day_bound_manifest_to_cross_day_schema(
    ...
    original = tmp_path / "home"
    original.mkdir()
    manifest_path = tmp_path / "runtime" / "home" / ".tinysoul" / "home_overlay.json"
