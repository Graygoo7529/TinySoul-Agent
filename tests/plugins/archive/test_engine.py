...
from tinysoul.plugins.archive.errors import (
...
THIRD_DAY = CalendarDay.parse("2026-07-13")
...
def test_daily_initial_journal_failure_discards_empty_pending_and_keeps_old_day(
    ...
    with pytest.raises(ArchiveInvariantError, match="journal"):
        coordinator.ensure_active_day(NEW_DAY, now=ROLLOVER_TIME)
...
def test_daily_resumes_after_trash_move_before_workspace_move(
    ...
    with pytest.raises(ArchiveInvariantError, match="Workspace"):
        coordinator.ensure_active_day(NEW_DAY, now=ROLLOVER_TIME)
...
def test_daily_resumes_final_archive_rename_failure(
    ...
    with pytest.raises(ArchiveInvariantError, match="finalize"):
        coordinator.ensure_active_day(NEW_DAY, now=ROLLOVER_TIME)
...
def test_finalized_legacy_transition_is_rejected(
    ...
    with pytest.raises(ArchiveInvariantError, match="journal fields are invalid"):
...
def test_pending_transition_with_home_requires_manual_recovery(
    ...
    with pytest.raises(ArchiveInvariantError, match="manual recovery"):
        coordinator.ensure_active_day(NEW_DAY, now=ROLLOVER_TIME)
...
def test_daily_lifecycle_rejects_overlapping_archive_before_initialization(
    ...
    with pytest.raises(ArchiveContractError, match="overlaps"):
        coordinator.ensure_active_day(NEW_DAY, now=ROLLOVER_TIME)
