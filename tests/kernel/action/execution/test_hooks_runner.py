...
async def test_runtime_transfer_joins_owner_and_retains_commit(peer_timeout) -> None:
    ...
    failure = RuntimeException(
        reason=HOME_RUNTIME_COPY_REQUIRED, message="copy required"
    )
    ...
    running = asyncio.create_task(
        ActionBatchRunner(executors=executors).run(
            batch,
            ActionExecutionContext(record_execution=facts.append),
        )
    )
    try:
...
async def test_cancel_interrupts_async_executor_and_preserves_prior_success() -> None:
    ...
    assert any(
        fact.result is not None and fact.result.payload == {"committed": True}
        for fact in facts
    )
...
async def test_runner_returns_failed_result_when_hook_rejects() -> None:
    ...
    assert results[0].failure.disposition is ActionFailureDisposition.CHANGE_REQUEST
...
async def test_runner_returns_local_failure_for_invalid_hook_outcome() -> None:
    ...
    result = (
        await ActionBatchRunner(executors=executors, hooks=hooks).run(
            batch,
            ActionExecutionContext(),
        )
    )[0]
