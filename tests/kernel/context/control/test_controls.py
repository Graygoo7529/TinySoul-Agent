...
from tinysoul.kernel.context.control.tools import (
...
from tinysoul.kernel.context.errors import ContextInvariantError
from tinysoul.kernel.context.signals import (
...
def test_normalize_failures_become_local_results() -> None:
    ...
    normalization = normalizer.normalize(
        (
            ...
            ToolCallRecord(
                id="dup", name=CONTROL_REMOVE_MILESTONE, arguments={"key": "a"}
            ),
    ...
    reasons = {
        result.call_id: result.frame_data.get("reason")
        or result.frame_data.get("tool_kind")
