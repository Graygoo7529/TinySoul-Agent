...
async def test_read_only_views_require_real_inspection_and_nonoverlapping_routes() -> (
    None
):
    ...
    descriptor = SegmentDescriptor(
        ...
        1,
        ("history:",),
        capabilities=frozenset({SegmentCapability.INSPECT}),
    )
    ...
    with pytest.raises(ContextContractError, match="overlap"):
        registry.register(
            ReadOnlySegmentRegistration(
                replace(descriptor, id="other", ref_prefixes=("history:turn/",)),
                provider,
            )
        )
...
def _registration(
    name: str, provider: CounterProvider, order: int = 1
) -> SegmentRegistration[int, int]:
    return SegmentRegistration(
        SegmentDescriptor(name, "test", SegmentSlot.WORKING, order),
        provider,
        f"context.{name}.increment",
        int,
        _decode,
    )
...
def _signal(name: str, turn: str, value: int) -> Signal:
    return Signal(
        name=f"context.{name}.increment",
        source="test",
        scope=RunScope().push(RunLevel.TURN, turn),
        payload={"increment": value},
    )


async def test_registered_updates_retry_fixed_batch_without_installing_other_views() -> (
    None
):
    ...
    prompt = TaskPrompt(
        guide_blocks=(PromptBlock.from_text("task_prompt:guide:test", "next"),)
    )
    assert [item.label for item in context.compose(prompt).messages].count(
        "counter"
    ) == 2
...
async def test_cancelled_preparation_does_not_install_candidates() -> None:
    ...
    views = (
        SegmentRegistry()
        .register(_registration("counter", provider))
        .for_turn(
    ...
    preparing = asyncio.create_task(
        views.prepare((_signal("counter", "turn_cancel", 9),))
    )
...
async def test_open_failure_closes_partial_views_and_keeps_cleanup_diagnostics() -> (
    None
):
    first, second = CounterProvider(), CounterProvider(fail_open=True)
    views = (
        SegmentRegistry()
        .register(_registration("first", first))
        .register(
...
def test_duplicate_metadata_and_routes_are_rejected_at_assembly() -> None:
    ...
    with pytest.raises(ContextContractError, match="core update"):
        context.register_segment(
            replace(registration, signal_name="context.working.patch")
        )
