...
async def test_day_preparation_yields_and_joins_before_cancellation() -> None:
    ...
    runner = RootScheduler(
        user_turn=_UserTurn(), reflection=day, day=day, bus=SignalBus(), trap=_trap()
    )
...
async def test_day_failure_at_request_boundary_enters_agent_trap() -> None:
        ...
        async def preflight(self, *, scope=None):
            ...
            if self.calls > 1:
                raise RuntimeArchiveBridge().from_archive_error(
                    ArchiveInvariantError("private location")
                )
            return await super().preflight(scope=scope)
    ...
    user = _UserTurn()
    runner = RootScheduler(
        user_turn=user, reflection=day, day=day, bus=SignalBus(), trap=_trap()
    )
...
async def test_program_dispatches_typed_requests_to_user_or_reflection() -> None:
    user = _UserTurn()
    ...
    reflection_handle = runner.submit_turn(
        ReflectionRequest(
    ...
    assert [request.scope for request in reflection.requests] == [ReflectionScope.HOME]
...
async def test_queued_cancellations_release_capacity_and_retain_by_completion_order() -> (
    ...
    user, reflection = _UserTurn(), _Reflection()
    runner = RootScheduler(
        user_turn=user,
        reflection=reflection,
        day=reflection,
    ...
    active = runner.submit_turn(
        UserTurnRequest("only this executes", request_id="final")
    )
    worker = asyncio.create_task(runner.run())
...
async def test_program_startup_reports_complete_reflection_availability() -> None:
    ...
    event = next(
        ...
        if event.name == "program.reflection.available"
    )
    assert event.payload == reflection.availability().to_json()
