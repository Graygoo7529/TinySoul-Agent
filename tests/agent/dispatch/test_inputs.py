...
from tinysoul.agent.dispatch.inputs import (
    InputCommandParser,
    InputDispatcher,
    InputEvent,
    InputIntentKind,
...
from tinysoul.agent.lifecycle.runtime_policy import build_agent_trap


def test_input_parser_separates_user_and_active_turn_input() -> None:
    parser = InputCommandParser()
    assert (
        parser.parse(InputEvent("hello"), turn_active=False).kind
        is InputIntentKind.USER_TURN
    )
    assert (
        parser.parse(InputEvent("more"), turn_active=True).kind
        is InputIntentKind.APPEND_INPUT
    )
...
@pytest.mark.parametrize(
    ("text", "kind"),
    [
        ("/reflection", InputIntentKind.REJECTED),
        ("/reflection home", InputIntentKind.REFLECTION),
        ("/reflection memory 2026-07-12", InputIntentKind.REFLECTION),
        ("/reflection memory tomorrow", InputIntentKind.REJECTED),
        ("/reflection memory", InputIntentKind.REJECTED),
        (
            "/reflection memory 2026-07-12 summarize knowledge",
...
async def test_daily_trigger_uses_calendar_day_before_rollover_and_deduplicates() -> (
    ...
    root = RootScheduler(
        ...
        day=reflection,
        bus=SignalBus(),
        trap=build_agent_trap(),
    )
    commands = AgentCommands(root)
    first = await commands.request_reflection(
        ReflectionRequest(
            scope=ReflectionScope.DAILY,
            trigger=ReflectionTrigger.SCHEDULED,
        )
    )
    repeated = await commands.request_reflection(
        ReflectionRequest(
            scope=ReflectionScope.DAILY,
            trigger=ReflectionTrigger.SCHEDULED,
        )
    ...
    requests = tuple(
        item.request for item in first if isinstance(item.request, ReflectionRequest)
    )
    assert [item.scope for item in requests] == [
...
async def test_all_ingress_uses_bounded_roots_and_the_active_inbox() -> None:
    ...
    try:
        ...
        queued = await dispatcher.submit(
