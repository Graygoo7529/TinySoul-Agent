...
from tinysoul.agent.observation.observations import ObservationSubscriptions
...
def _event(text: str = "value", *, name: str = "changed") -> ObservationEvent:
    return ObservationEvent(
        name, ObservationLevel.NORMAL, "test", payload={"text": text}
    )
...
async def test_slow_observer_gets_gap_and_newest_bounded_records() -> None:
    ...
    assert [
        item.sequence for item in retained if isinstance(item, ObservationRecord)
    ] == [999, 1000]
    hub.close()
...
async def test_stream_serialization_failure_does_not_escape_observation_boundary() -> (
    None
):
