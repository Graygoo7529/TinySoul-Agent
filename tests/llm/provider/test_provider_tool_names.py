...
def test_provider_tool_name_map_keeps_safe_names_and_maps_dotted_names() -> None:
    name_map = ProviderToolNameMap.from_names(
        ("read_file", "workspace.list", "1leading", "\u5de5\u5177.read")
    )

    assert name_map.to_provider_name("read_file") == "read_file"
    assert name_map.to_provider_name("workspace.list") == "workspace_list"
    assert name_map.to_provider_name("1leading") == "_1leading"
    for tinysoul_name in ("workspace.list", "1leading", "\u5de5\u5177.read"):
...
def test_provider_tool_name_map_resolves_alias_collisions() -> None:
    name_map = ProviderToolNameMap.from_names(
        ("workspace.list", "workspace_list", "workspace/scan")
    )

    provider_names = {
        name_map.to_provider_name(name)
        for name in ("workspace.list", "workspace_list", "workspace/scan")
    }
    assert len(provider_names) == 3
    assert name_map.to_provider_name("workspace_list") == "workspace_list"
    for provider_name in provider_names:
        assert (
            name_map.to_provider_name(name_map.to_tinysoul_name(provider_name))
            == provider_name
        )
