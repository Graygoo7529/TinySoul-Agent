...
def test_tool_selection_validates_names() -> None:
    ...
    assert (
        ToolSelection(("read_file",), forced_name="read_file").forced_name
        == "read_file"
    )
...
def test_default_tool_call_id_mapper_keeps_valid_provider_id() -> None:
    ...
    assert mapper.to_tinysoul_id("call_1", index=0, tool_name="read_file") == "call_1"
...
def test_default_tool_call_id_mapper_generates_provider_friendly_id() -> None:
    ...
    assert (
        mapper.to_tinysoul_id("1 bad", index=1, tool_name="read/file") == "read_file_2"
    )
