...
from tinysoul.llm.execution.context_window import RequestTokenEstimate
...
from tinysoul.llm.execution.model_chain import (
    Clock,
    LLMRouteState,
    Sleeper,
    TaskSpecTable,
...
async def test_runner_observations_share_stable_task_id() -> None:
    ...
    assert all(
        event.payload["task_id"] == "task_visible" for event in observations.events
    )
...
async def test_backup_provider_preference_expires_back_to_chain_head() -> None:
    ...
    runner = LLMTaskRunner(
        ...
        providers=ProviderRegistry([primary, backup]),
        tasks=_tasks(
            ModelChain(profile="framework", model_ids=("shared",), retry_policy=policy)
        ),
        route_state=state,
        clock=clock,
    )
    ...
    clock.current = 1
    (await runner.run(call))
    assert [request.binding.provider_id for request in backup.requests] == [
        "backup",
        "backup",
    ]
...
async def test_owner_cancellation_interrupts_and_joins_backoff(
    max_retries: int,
) -> None:
...
async def test_repeated_task_cancellation_joins_provider_finalizer() -> None:
    ...
    runner = LLMTaskRunner(
        models=_models("a"),
        providers=ProviderRegistry([Provider(provider_id="fake")]),
        tasks=_tasks(ModelChain(profile="framework", model_ids=("a",))),
    )
...
class FixedTokenEstimator:
    ...
    def estimate(
        self, messages: MessageStack, tool_scope: ToolScope
...
async def test_context_hard_water_requests_runtime_recomposition_before_provider() -> (
    None
):
    provider = FakeProvider(provider_id="fake")
    runner = LLMTaskRunner(
    ...
    assert exc_info.value.payload["kind"] == LLMFailureKind.MODEL_CONTEXT_PRESSURE
...
async def test_smaller_fallback_requests_recomposition_without_chain_checkpoint() -> (
    None
):
    @dataclass
...
async def test_runner_switches_model_without_retry_for_non_transient_provider_error() -> (
    None
):
    @dataclass
...
async def test_runner_reports_successful_fallback_model_during_preference_window() -> (
    None
):
    provider = FakeProvider(provider_id="fake", failures={"a": 1})
    clock = FakeClock()
...
async def test_runner_capability_query_returns_to_head_after_preference_expires() -> (
    None
):
    provider = FakeProvider(provider_id="fake", failures={"a": 1})
    clock = FakeClock()
...
async def test_runner_returns_failure_result_for_json_parse_error() -> None:
    ...
    assert (
        "Failed to parse model response as JSON object" in result.failure.model_feedback
    )
...
async def test_model_chain_exhaustion_payload_reports_non_transient_provider_error() -> (
    None
):
    @dataclass
    class ConfigFailingProvider(FakeProvider):
...
async def test_classified_control_preserves_identity_without_retry(
    control: Exception,
) -> None:
...
async def test_runner_reports_model_chain_exhaustion_without_routable_provider() -> (
    None
):
    model = ModelSpec(
        id="model_a",
