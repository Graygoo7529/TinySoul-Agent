...
from tinysoul.infra.config.project_file import ProjectConfigFile


def test_project_config_reads_toml_as_dotted_source(local_tmp: Path) -> None:
    ...
    source = ProjectConfigFile(path).to_source()

    assert source.values["infra.runtime.max_turns"] == 20
    assert source.values["infra.runtime.parallel_workers"] == 5


def test_project_config_set_value_and_save_round_trips(local_tmp: Path) -> None:
    path = local_tmp / "tinysoul.toml"
    config = ProjectConfigFile(path)
    ...
    reloaded = ProjectConfigFile(path).to_source()
...
def test_project_config_data_returns_copy(local_tmp: Path) -> None:
    path = local_tmp / "tinysoul.toml"
