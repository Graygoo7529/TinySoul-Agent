...
- temp files with the temp_yymmdd_ prefix are identified by naming convention; temp_yymmdd_content.md is markdown, temp_yymmdd_script.py is py(future)
...
（2）action_cluster：<BASIC, FILE>

（3）action_profile：
- action_type：INTERNAL_CALL
...
- edge_case_handling：[
    ...
    "Path traversal outside workspace: reject with error",
    "Missing context resource: reject with error"
  ]
...
- Step 2a: generate action arguments
  ...
  - 完整的 content / new_content 不在 Step 2 中生成
...
- Step 2b: execute action（Step 2 的后续执行阶段）
  ...
  - execute 函数会：
    ...
    5. 调用 llm 生成完整的 content（create_markdown_file）或 new_content（edit_markdown_file）
