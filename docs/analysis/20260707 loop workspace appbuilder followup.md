...
本文记录 Loop 模块中与 Workspace、Agent Home 后续模块相关的遗留装配问题。当前 Loop 已完成运行编排与 TinySoulApp 装配入口，以下内容不是当前功能错误，而是后续推进 Workspace / Agent Home 资源读写机制时需要优先处理的边界。

## 当前实现位置

- `tinysoul/loop/app.py` 的 `TinySoulAppBuilder` 负责装配 LLM、Action、Context、SignalBus、RuntimeTrap、输入监听器和各级 runner。
- `workspace.scan` 当前作为内置 native action 在 `TinySoulAppBuilder._build_action()` 中注册。
- `workspace.scan` 的实现直接位于 `tinysoul/loop/app.py`，扫描项目根目录，生成 `workspace:` 链接和简短摘要，并通过 `context.working.patch` 更新 WorkingContext。
...
Loop 的职责是运行编排与装配，不应长期承担 workspace 文件扫描、Agent Home 内容读取、运行时副本管理、how_action 检索或每日沉淀策略。

当前把 `workspace.scan` 放在 `app.py` 中，是 Workspace 模块未落地前的轻量装配能力：它让 Phase3 可以通过 action -> signal 的既有通道向 WorkingContext 写入 workspace 资源摘要，验证了 Loop、Action、Context 的协作路径。

## 风险与不足

- `workspace.scan` 的扫描规则、跳过目录、数量上限、摘要格式都写在 Loop 装配层，属于 Workspace 语义泄漏。
- `TinySoulAppBuilder` 的 `_build_action()` 同时承担 action catalog 装配、native action 注册、workspace 扫描函数闭包构造和 nested LLM executor 注册，随着 Workspace / Agent Home 接入会继续膨胀。
...
- `workspace.scan` 迁移为 Workspace 模块提供的 native action 或 Action executor 注册材料；Loop 只负责调用 builder 的注册方法或接收已装配好的 ActionEngine。
...
- AppBuilder 应保持全局装配入口，但把模块专属构造逻辑下沉到对应模块 builder，避免 `app.py` 累积业务语义。
