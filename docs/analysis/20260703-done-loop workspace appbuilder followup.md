...
- App 层旧的 `workspace.scan` 临时实现和 app-owned `core.answer` 已清除；`core.answer` 由 Action builtins core executor 提供，app native-action 临时层已删除。
...
AppBuilder 只承担跨模块装配入口，通过模块 registrar 注册 Workspace、Agent Home 与 Action builtins core executor，并装配 LLM action backend service；具体扫描、摘要刷新、workspace 写入和回答动作语义已经下沉到对应模块。这个通道继续验证 Phase3 可以通过 action -> signal 向 WorkingContext 写入 workspace 资源摘要，但 App 不再拥有 workspace 业务实现。
...
- Workspace 已提供 `WorkspacePromptInput`，可将一个或多个 `workspace:` 文本资源按有界前缀读取并渲染为临时 task prompt 输入；该能力不暴露为模型侧 `workspace.read` action。
