...
状态：`in_progress`（R1 已完成；R2 原实施批次已归档，2026-09-16 复审发现生命周期、统一等待与服务边界缺口，转入 R2 收口子计划。S1 保持完成，S2 恢复进行中；S3–S7 未整体完成，保持原定范围）。
...
| S2 `in_progress` | 新内核/SDK/CLI/段/Job/等待；同步迁移所有旧内核消费者至新公共入口、插件接入与打包 | R2 基线 Full 1065 passed、typecheck 通过；完整验收待 R2 收口计划关闭生命周期、等待、服务权限与终态/保留缺口 |
