...
4. Session Turn 持久 preview 继续包含 ask、answer、完整 Action 聚合、可选 Action outcome detail 和 trace digest。`background_action_names` 是 Session-owned 的有界持久审计选择，不复制任意 ActionResult payload；自动 Context 另从同一已验证 Action projection 构造逐 Action outcome，最终 answer 由 Turn output 独立投影。
...
- 下一 Turn 的固定 Session Background 能看到之前 Turn 的 ask、answer、references 与逐 Action success/failed/timeout；配对异常只显示对应 Action 的 `incomplete_count`，完整审计和配置选择的 occurrence detail 只由显式查询提供。
...
- Session 自动 Background 已进一步收敛为 kind/ref、ask、answer、references、exhausted 和逐 Action 聚合 outcome；完整 trace/pairing summary、turn id 与配置选择的 occurrence detail 不再自动交付模型。持久 v3 preview 和 Endpoint/显式 Session 查询保持原协议，现有运行记录不需要迁移。
