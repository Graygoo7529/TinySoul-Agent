...
tinysoul/app/
  ...
  runtime_policy.py  # Program-only Runtime trap
...
可编辑项目模板位于 `tinysoul/assets/project/` 并作为 package data 发布；User Action Catalog 的
bootstrap template 位于 `tinysoul/action/catalog/`，Maintenance 专用 fragment 位于
...
### Configuration Plan and Generation

`TinySoulAppBuilder` 先从统一 `ConfigEnvironment` 编译 `AppConfigPlan`。Plan 只保存各模块
parser 产生的 typed settings，App 不解释模块内部字段；显式注入的测试 LLM/Settings 继续由
builder override。`RuntimeGenerationFactory`（当前由 builder 的 `_build_generation` 承担）
使用同一 Plan 构造完整 `AppRuntimeGeneration`，首次启动和配置 PATCH 不再有两套装配路径。
...
AppBuilder 是跨模块配置装配边界，但配置错误归属仍属于对应模块。ConfigEnvironment 加载 merged
section sources，并携带独立 document-set 快照；Infra、Context、LLM、Loop、App、Session、
...
协调二者并在候选 Generation 建立前完成引用校验。配置页面使用的标题、说明、字段层级、document
field、引用提示和受控对象创建 source 全部由 `infra.config` 的 package catalog 提供；App 不聚合
descriptor，不缓存配置对象投影，也不判断前端页面。Endpoint 对有效/不可用 Action 的读取仍通过
当前 Runtime Generation 的只读门面。
