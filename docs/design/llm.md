...
模型链采用顺序尝试策略。一次调用从当前起点开始沿模型链向后尝试；某个模型发生暂时性供应商错误后继续尝试链上的下一个模型。走到链尾仍失败时，才进入下一轮并重新从链头开始。尝试轮数耗尽后进入 Runtime 语义陷入，由运行层决定结束当前 Turn 或执行其他上层处理。
...
LLM 模块与 Runtime 的语义桥接通过独立桥接层完成。LLM 内部可以使用自身错误类型表达能力不满足、调用契约错误、供应商错误和解释错误；跨出模块边界并需要改变运行控制流时，桥接层将这些失败转换为 Runtime 可理解的稳定原因。模型链耗尽映射为 LLM 模型链耗尽原因，由 Trap 处理为结束当前 Turn。调用契约错误、能力检查失败、非暂时性供应商错误等不能作为任务失败反馈继续喂给模型的情况，映射为无法处理的运行失败。
