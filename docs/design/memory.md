...
配置的 Memory root 在尚无任何记忆时可以不存在；Background/search 将其视为空 store，recall 返回 not-found。只有 Memory Maintenance 成功写入时才可创建 root 与目标年/月父目录；模块 import、builder、search 和 recall 不得为只读请求产生目录副作用。root 已存在但不是目录、越出项目边界或 Maintenance 写入时不可用，属于 Memory 边界失败。
...
每份 MEMORY 由框架确定性渲染日期标题与三个中文时间段：

- 上午 `[00:00, 12:00)`；
- 下午 `[12:00, 18:00)`；
- 晚上 `[18:00, 24:00)`。

Turn 整体归入其开始时间所在段，不因执行跨段而拆分。LLM 只返回三个 Markdown body，Memory renderer 拥有最终文档结构。文档必须在 Memory 配置的总长度上限内；超限不以静默截断伪装成完整记忆。
...
- 文件存在时返回 Link 和完整、受上限约束的 Markdown；
- 文件无法读取、无法解释或超过完整文档上限时，preparation 以 Memory 模块失败结束，不伪装为“昨日无记忆”。
...
`memory.search(query, top_k)` 对 Memory-owned 日期文档执行有界检索。候选的语义单元是三个时间段，但最终 item 按日期 Link 去重：每个日期只返回最相关时间段。每个 item 只包含：
...
Memory 负责确定性候选、候选上限、稳定排序和 candidate-only validator；可使用专用 `memory_search` LLM Task 重排候选。Task 失败或返回候选外 Link 时回退确定性结果，不把只读检索变成不可用。Search 不返回完整 MEMORY，也不修改 Background。

### Recall

`memory.recall(memory_link)` 只接受精确 `MemoryLink`，返回该日期的完整 Markdown 和稳定元数据。Recall 不提供时间段过滤或分页；Maintenance 产生的文档已被总长度上限束缚。若外部修改使文档超限，recall 显式失败而不返回被截断且伪称完整的正文。
...
Session 负责归档图校验、按需递归 Summary 和去重可达 Turn 事实；Memory 不读取 Session store 或 archive 内部文件。Memory consolidator 按三个时间段分组，将 Session facts 与可选同日期旧 MEMORY 执行有界分块和分层 reduce；它不读取其它日期 MEMORY 正文、Workspace、active Home diff 或 `SKILL_MEMORY.md`。
...
`MemoryEngine` 是唯一组装门面，对上层提供 Link/store 查询、Background provider、search/recall、eligibility 和 Maintenance。内部 service 不作为 App/Loop 的平行组装入口。

Memory 配置位于顶层 `[memory]`，拥有根目录、完整文档上限、search 预算和 consolidation 预算。旧 `[home.memory]` 整体删除；Memory 不继承 Home `max_write_chars` 或 Home root。配置加载时不接受旧键别名。
...
2. 模块边界异常：配置无法解释、Memory root 不可用、已存文档损坏/超限、路径不变量破坏或原子写失败；
...
- `memory.search` 返回去重日期 Link、最相关时间段和有界摘要，不返回完整正文；
...
- Memory Maintenance 只读取指定日 Session projection 和可选同日旧 Memory，成功时原子完整重写；
