...
Infra 当前优先负责配置环境机制，后续可继续承载日志、资源读取、进程运行、能力探测等基础能力。每项基础能力都应保持小而明确的边界，避免反向了解 Loop、Action、LLM 或 Workspace 的业务细节。
...
项目配置可以由主配置文件和显式声明的多个配置文件组成。主配置文件负责作为项目配置入口，其他配置文件可按模块或主题组织。Infra 只负责读取和合并这些文件，不解释其中的领域语义。

## 可写配置

项目需要适合人阅读和编辑的配置文件，承担长期、非敏感配置的存储角色。

配置文件应支持分组，使不同模块的配置自然归属到自己的命名空间。配置可以按主题拆分到多个文件，但加载顺序必须显式、可解释。写入配置时应保持内容稳定、可预测，并避免把密钥或临时环境值写入项目配置。
...
统一配置环境应同时提供配置树视角和类型化分段加载视角。结构化模块配置可以读取自己的配置树并自行解析；简单配置可以直接加载为类型化设置。两种读取方式应来自同一套来源和优先级规则，而不是两套独立配置机制。

## 当前取舍

配置环境机制先作为 Infra 的第一个落地点。其他基础设施可以在需要时逐步建立，但不应为了未来可能需要而提前创建没有实际使用场景的抽象。
