...
Infra 自身只抛出配置和基础设施语义的错误，不直接表达 Runtime 控制流。应用入口、组合层或专门桥接层负责把启动阶段的配置错误转换为 Runtime 可理解的启动失败原因，再交给 Trap 决定结束 Program 或执行其他上层处理。这样 Infra 保持纯基础设施边界，Runtime 也不会反向侵入配置加载实现。
