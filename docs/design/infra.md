...
项目配置由 `tinysoul.toml` 作为入口，显式 include `configs/*.toml` 与 `configs/llm.models/*.toml`。app/action/context/home/loop/workspace 和 LLM provider/task 分别保存在对应配置文件中。glob 展开顺序稳定，Infra 只负责读取和合并这些文件，不解释其中的领域语义；模块 parser 在实际模块边界把 section tree 转成 Settings。
...
Infra 自身只抛出配置和基础设施语义的错误，不直接表达 Runtime 控制流。Infra 内部维护服务于 Runtime bridge 的稳定失败类型，用于表达需要跨出基础设施边界并改变运行控制流的配置失败、JSON 边界失败、调用契约错误和内部失败。应用入口、组合层或专门桥接层负责通过映射表把这些失败转换为 Runtime 通用原因：启动阶段的配置错误映射为启动失败，运行阶段无法在模块内处理的基础设施失败默认映射为结束当前 Turn。这样 Infra 保持纯基础设施边界，Runtime 也不会反向侵入配置加载实现。
