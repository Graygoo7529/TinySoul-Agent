...
模块边界负责决定失败的去向。可以在模块内部修复的失败，应由模块内部重试、切换、降级或转换为模块结果；需要反馈给模型或上层业务流程继续处理的失败，应返回结构化模块结果，例如 LLM TaskFailure 当前通过 `model_feedback` 提供可供上层加入后续模型提示的错误信息；需要改变运行控制流的失败，才通过 Runtime bridge 转换为 Runtime 语义异常。默认跨模块失败映射为结束当前 Turn；启动装配阶段的失败映射为启动失败；只有需要独立全局恢复例程时，才新增 Runtime 原因。
