...
Program 是程序顶层，负责等待用户输入、执行退出指令，以及调度与用户轮同级的其他任务。Program 只直接调度 Turn，不直接介入 Phase 或具体模块细节。

Turn 是 Program 下的一次顶层任务。用户输入形成 User Turn，每日沉淀或维护任务也可以形成与 User Turn 同级的 Turn。不同类型 Turn 的调度策略可以不同，例如 User Turn 可以接收用户追加输入，Daily Turn 执行期间不接收用户输入；但它们在运行控制层级上同属 Turn。
...
当局部处理失败，或发生需要全局协调的情况时，模块边界抛出 Runtime 可理解的语义异常，异常进入 Runtime 陷入流程。Runtime 语义异常使用稳定原因标识表达陷入原因，例如启动失败、无法处理的运行失败、Turn 中断请求、Program 结束请求、语境压缩请求或 Agent Home 运行时副本准备请求。Trap 将异常和当前运行位置转换为 TrapSnap，并按照原因标识查找处理器。

Runtime 的异常入口应保持单一。模块外交给 Runtime 的异常使用统一异常类型承载原因标识、错误消息和结构化载荷。Runtime 不通过庞大的异常继承树区分恢复、中断和退出，而是通过处理器注册表解释原因标识。这样可以保持异常入口稳定，并允许 Context、Home、LLM、Action 等模块以各自命名空间扩展 Runtime 陷入原因。

TrapSnap 是 Trap 捕获异常后形成的陷入上下文快照。它包含原因标识、错误消息、结构化载荷、运行位置和可选 traceback。TrapSnap 不再被抛出；它只在 Trap 处理器、日志、TurnTrace 和可观测流程中流动。

常见原因包括：程序启动失败、模块边界无法处理的运行失败、Turn 中断、Program 结束、需要语境压缩、需要 Agent Home 运行时副本准备。具体原因名称属于模块间协议，应稳定、可记录、可测试。
...
运行器负责消费运行转移。Program 运行器处理 Program 和 Turn 级转移；Turn 运行器处理 Turn、Cycle 和 Phase 级转移；模块运行边界处理 Module 级转移，并在无法消费时向上层传播。Runtime 本身不直接驱动业务模块重跑，也不直接提交语境状态。
...
Trap 处理器负责解释原因标识。它可以直接返回结束 Turn、结束 Program 等转移，也可以执行全局恢复例程后返回重试某个 frame 的转移。新增模块级陷入原因不应要求修改 Runtime 主控制流程，而应通过注册处理器接入。处理器可以读取 TrapSnap，但不应反向依赖具体业务模块内部状态。

通用处理策略包括：启动失败结束 Program；无法处理的运行失败结束当前 Turn；Turn 中断请求结束当前 Turn；Program 结束请求结束 Program；语境压缩或 Agent Home 运行时副本准备等原因由对应处理器执行恢复后返回运行转移。未知异常被记录为非预期故障，并按当前运行层级选择结束 Turn 或结束 Program。
