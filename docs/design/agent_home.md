...
Agent Home 不维护 Turn 内 Context 状态，不驱动 Loop，不执行模型供应商调用，也不管理 workspace 文件。它向 Context 提供可加载的顶层背景内容，向 Loop 提供 domain guidance provider，向 Action 提供 home 资源读取和编辑动作，并通过 Runtime 恢复例程维护当日 runtime home 副本。
...
4. Loop 只依赖 `DomainGuidanceProvider` 协议，不读取 HOW 文件。
...
- `home:how_action/workspace/examples.md`

链接规则：

- `space` 是稳定命名空间，例如 `agent`、`what`、`why`、`how`、`how_action`、`memory`；
...
- `home:how_action@domain_name` 映射到 `how_action/domain_name/DOMAIN.md`；
- `home:memory@yyyy-mm-dd` 映射到 `memory/yyyy-mm-dd.md` 或当日 memory 摘要。

HOW 采用包目录形式。`SKILL.md` 是顶层入口，references、scripts 等是渐进式资源。与 action domain 绑定的 HOW 使用 `how_action/<domain>/DOMAIN.md`，由 Phase2 prompt 自动注入。
...
Loop 已定义 `DomainGuidanceProvider`。Agent Home 应提供真实实现，例如 `HomeDomainGuidanceProvider`。

Provider 的职责：

1. 接收 Phase1 已选择的 action domain；
2. 将 domain 映射为 `home:how_action@<domain>`；
3. 读取对应 domain 的 `DOMAIN.md`；
...
- 追加或修改 MEMORY；
- 合并 WHAT、WHY、HOW 或 how_action 的变更；
...
`AgentHomeEngine` 是上层唯一门面，提供链接解析、顶层背景条目、渐进式资源访问、runtime copy 和 domain guidance。`AgentHomeEngineBuilder` 负责接收已解析设置、校验目录、装配布局和 runtime copy manager。每日沉淀资源操作尚未落地，后续可在 `settlement.py` 或更明确的维护任务模块中加入。

AppBuilder 的目标职责是：

1. 构建 AgentHomeEngine；
2. 将 Home 提供的默认背景和 loadable 背景交给 ContextEngineBuilder；
3. 将 HomeDomainGuidanceProvider 注入 Phase2Unit；
