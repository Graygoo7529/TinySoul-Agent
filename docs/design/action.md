...
`llm_step` action 的业务参数至少需要提供非空 `guide`。任务输入的主要语义面是可切分的 `task_inputs` 与 `references`：`task_inputs` 由若干 `{label, text}` 块组成，并渲染为多条 `PromptBlock`；`references` 由注入的 `PromptReferenceResolver` 解析为 `PromptBlock`，例如 Workspace 模块提供 `workspace.text` 引用，把 workspace 文本前缀或行范围切片转为临时 task prompt。旧的 `task_input` 字符串只保留为兼容入口，执行器会把它转换成一个 PromptBlock，不作为新的主要协议继续扩展。

嵌套 LLM task 固定要求 JSON object 输出，并禁用模型侧工具调用；成功时 JSON object 作为 action payload 返回，Context 构造失败、Runtime 语义异常、引用解析失败、LLM task failure 或非 JSON object 输出都收敛为 execute 阶段的 `ActionResult`。内置 `core.reason` 使用 `llm_step.context_task` handler，作为通用只读推理动作；它可以携带 workspace references，但不会把 workspace 正文写入普通 ActionResult。
