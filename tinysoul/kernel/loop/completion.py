...
from .inbox import InboxError, InboxKind, QuestionRequest, WaitCondition, WaitReason
...
class WaitRequest:
    ...
    def __post_init__(self) -> None:
        ...
        if self.timeout_seconds is None and self.event_kind is None and self.job_id is None:
    ...
    def condition(self, after_sequence: int) -> WaitCondition:
        return WaitCondition(
            WaitReason.EVENT if self.event_kind is not None or self.job_id is not None else WaitReason.TIMER,
            after_sequence,
            asyncio.get_running_loop().time() + self.timeout_seconds if self.timeout_seconds is not None else None,
            self.event_kind, self.event_id, self.job_id,
        )


def wait_from_results(results: tuple[ActionResult, ...]) -> WaitRequest | None:
    waits = [result for result in results if result.action_name in {"core.wait", "core.job.wait"}
    ...
    event_id, job_id, ready = payload.get("event_id"), payload.get("job_id"), payload.get("ready", False)
    if ((timeout is not None and type(timeout) not in (int, float))
            or any(value is not None and not isinstance(value, str) for value in (kind, event_id, job_id))
    ...
    try:
        request = WaitRequest(
            ...
            job_id if isinstance(job_id, str) else None, ready,
        )
        request.condition(0)
        return request
