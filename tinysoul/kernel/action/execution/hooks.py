...
from ..catalog.schema import ActionSchemaValidationError, validate_action_params
...
class HookOutcome:
    ...
    def __post_init__(self) -> None:
        ...
        if "failure" in payload:
            raise ActionInvariantError("HookOutcome.payload cannot contain failure")
    ...
    def check(self, item: ActionNormalizeInput) -> HookOutcome:
        try:
            validate_action_params(
                item.tool_call.arguments, schema=item.action.tool.schema
            )
...
class ActionHookRegistry:
    ...
    def register_normalize_hook(self, name: str, hook: ActionNormalizeHook) -> None:
        ...
        if name in self._normalize_hooks:
            raise ActionInvariantError(
                f"Action normalize hook already registered: {name}"
            )
    ...
    def register_execution_hook(self, name: str, hook: ActionExecutionHook) -> None:
        ...
        if name in self._execution_hooks:
            raise ActionInvariantError(
                f"Action execution hook already registered: {name}"
            )
    ...
    def normalize_hook_for(self, name: str) -> ActionNormalizeHook:
        try:
            ...
            raise ActionContractError(f"Unknown action normalize hook: {name}") from exc

    def execution_hook_for(self, name: str) -> ActionExecutionHook:
        try:
            return self._execution_hooks[name]
        except KeyError as exc:
            raise ActionContractError(f"Unknown action execution hook: {name}") from exc
