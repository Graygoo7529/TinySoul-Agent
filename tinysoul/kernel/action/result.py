...
_FAILURE_IDENTIFIER_CHARS = frozenset("abcdefghijklmnopqrstuvwxyz0123456789._-")
...
class ActionLocalFailure:
    ...
    def from_json(cls, value: object) -> "ActionLocalFailure":
        ...
        if set(value) - {"reason", "scope", "disposition", "feedback", "constraint"}:
            raise ActionInvariantError(
                "Action failure envelope contains unknown fields"
            )
...
class ActionResultEnvelope:
    ...
    def __post_init__(self) -> None:
        ...
        _validate_failure_status(
            self.status, self.failure, owner="ActionResultEnvelope"
        )
        object.__setattr__(
            self,
            "payload",
    ...
    def from_json(cls, value: object) -> "ActionResultEnvelope":
        ...
        if set(value) - {"action", "status", "stage", "payload", "failure"}:
            raise ActionInvariantError("Action result envelope contains unknown fields")
        ...
        if not isinstance(payload, dict):
            raise ActionInvariantError(
                "Action result envelope payload must be an object"
            )
...
class ActionResult:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.stage, ActionResultStage):
            raise ActionInvariantError(
                "ActionResult.stage must be an ActionResultStage"
            )
        if self.sequence <= 0:
