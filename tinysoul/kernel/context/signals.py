...
def parse_trace_append_signal(signal: Signal) -> TraceAppend:
    ...
    if kind is TraceAppendKind.ACTION_RESULT:
        ...
        origin_refs_value = signal.payload.get("origin_refs", [])
        if not isinstance(origin_refs_value, list) or any(
            not isinstance(ref, str) or not ref for ref in origin_refs_value
        ):
            raise ContextContractError(
...
class InputAppend:
    ...
    def __post_init__(self) -> None:
        if not self.text or not self.input_id or not isinstance(self.reply_to, str):
            raise ContextContractError("Input update requires text and stable identity")
