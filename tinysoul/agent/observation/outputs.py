...
class ObservationRouter:
    ...
    def mode(self) -> ObservationLevel:
        return max(
            (self._mode, *(route.mode for route in self._routes)), key=_level_rank
        )
    ...
    def enabled(self, level: ObservationLevel) -> bool:
        with self._lock:
            return self.subscriptions.enabled(level) or any(
                _level_rank(level) <= _level_rank(route.mode) for route in self._routes
            )
            ...
            if not self._subscriptions_disabled:
                try:
                    ...
                    self._failures.append(
                        CleanupDiagnostic(
                            "observation.subscriptions", type(exc).__name__
                        )
                    )
