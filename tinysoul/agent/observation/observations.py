...
class ObservationFilter:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.turn_id, str) or any(
            not isinstance(name, str) or not name for name in self.names
        ):
    ...
    def accepts(self, event: ObservationEvent) -> bool:
        ...
        return (
            tuple(ObservationLevel).index(event.level)
            <= tuple(ObservationLevel).index(self.level)
...
class ObservationRecord:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            or self.sequence < 1
            or not isinstance(self.event, ObservationEvent)
        ):
            raise AgentSDKError(
                "Observation record requires a positive sequence and event"
            )
...
class ObservationGap:
    ...
    def __post_init__(self) -> None:
        if any(
            ...
            for value in (self.first_sequence, self.last_sequence, self.dropped_count)
        ):
            raise AgentSDKError("Observation gap requires integer bounds")
        if (
            not 1 <= self.dropped_count <= self.last_sequence - self.first_sequence + 1
...
class ObservationSubscription:
                ...
                self._records.append(
                    (
                        replace(
                            ...
                            event=replace(
                                record.event,
                                payload=to_json_object(record.event.payload),
                            ),
                        ),
                        size,
                    )
    ...
    def _drop(self, sequence: int) -> None:
        ...
        self._gap = ObservationGap(
            ...
            sequence,
            previous.dropped_count + 1 if previous is not None else 1,
        )
...
class ObservationSubscriptions:
        ...
        *,
        ...
        with self._lock:
            ...
            subscription = ObservationSubscription(
                self, selection, capacity=capacity, max_bytes=max_bytes
            )
    ...
    def enabled(self, level: ObservationLevel) -> bool:
        with self._lock:
            return any(
                tuple(ObservationLevel).index(level)
                <= tuple(ObservationLevel).index(item.selection.level)
