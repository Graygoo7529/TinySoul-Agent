...
class InputIntent:
    ...
    text: str = ""
    source: str = ""
    reflection_scope: ReflectionScope | None = None
    ...
    def __post_init__(self) -> None:
        ...
        if self.kind is InputIntentKind.REFLECTION:
            if not isinstance(self.reflection_scope, ReflectionScope):
                raise AgentContractError("Reflection intent requires a scope")
        elif self.reflection_scope is not None:
...
class InputCommandParser:
        ...
        if normalized.split()[0] in {"/reply", "/grant"}:
            ...
            if len(parts) != 3:
                return self._intent(
                    ...
                    event,
                    error="Use /reply QUESTION_ID TEXT or /grant REQUEST_ID COUNT.",
                )
            if parts[0].casefold() == "/reply":
                return replace(
                    self._intent(InputIntentKind.REPLY, event, text=parts[2]),
            ...
            if (
                ...
                return self._intent(
                    InputIntentKind.REJECTED,
                    event,
                    error="Cycle grant must be a positive integer.",
                )
            return replace(
                self._intent(InputIntentKind.GRANT, event),
                correlation_id=parts[1],
                cycles=int(parts[2]),
            )
        if normalized.startswith("/reflection"):
            return self._reflection(event, text)
    ...
    def _reflection(self, event: InputEvent, text: str) -> InputIntent:
        parts = text.split(maxsplit=2)
        if len(parts) < 2:
        ...
        if kind == "home":
            return self._intent(
                InputIntentKind.REFLECTION,
                event,
                text=remainder,
                reflection_scope=ReflectionScope.HOME,
            )
        if kind == "memory":
            ...
            return self._intent(
                ...
                event,
                text=instructions,
                reflection_scope=ReflectionScope.MEMORY,
                target_day=target,
            )
        return self._rejected(event, text)
    ...
    def _rejected(self, event: InputEvent, text: str) -> InputIntent:
        return self._intent(
            ...
            text=text,
            error=(
                "Use /reflection home [instructions] or "
                "/reflection memory YYYY-MM-DD [instructions]."
            ),
        )
...
class InputDispatcher:
    ...
    async def submit(self, event: InputEvent) -> CommandReceipt:
        active = self._commands.active_turn
        parser = (
            self._parser_provider()
            if self._parser_provider is not None
            else self._parser
        )
        ...
        try:
            ...
            if intent.kind is InputIntentKind.USER_TURN:
                await self._commands.submit_turn(
                    UserTurnRequest(
                        intent.text,
            ...
            if intent.kind is InputIntentKind.REFLECTION:
                assert intent.reflection_scope is not None
                await self._commands.request_reflection(
                    ReflectionRequest(
                        scope=intent.reflection_scope,
            ...
            state = (
                "full"
                if isinstance(exc, (AgentQueueFullError, InboxCapacityError))
                else "rejected"
            )
        ...
        *,
        ...
        try:
            ...
            return CommandReceipt(
                False, command_id, InputIntentKind.REFLECTION.value, "full"
            )
        return CommandReceipt(
            True, command_id, InputIntentKind.REFLECTION.value, "queued"
        )
