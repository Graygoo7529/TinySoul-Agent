...
class TerminalInputSource:
        ...
        try:
            for chunk in self._chunks():
                ...
                while "\n" in pending:
                    ...
                    sink.submit(InputEvent(text=line.rstrip("\r") + "\n", source="terminal"))
                if len(pending) > 64_000:
                    raise EnvironmentError("Terminal input line exceeds its bounded buffer")
    ...
    def _chunks(self) -> Iterator[str]:
        ...
        decoder = codecs.getincrementaldecoder(self._stream.encoding or "utf-8")(errors="replace")
        try:
            while not self._stop_event.is_set():
                try:
