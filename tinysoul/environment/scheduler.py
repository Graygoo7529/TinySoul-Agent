...
from tinysoul.plugins.reflection import (ReflectionSchedule, ReflectionScheduleSettings, ReflectionRequest)
...
class ReflectionScheduler:
    ...
    def _run(
        ...
        while not self._stop_event.is_set():
            ...
            pending = tuple(request for request in pending if not sink.submit_request(request))
            wait_seconds = min(1.0, schedule.seconds_until_next(now)) if pending else schedule.seconds_until_next(now)
