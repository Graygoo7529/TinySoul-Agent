"""Project-level TOML configuration file support."""
...
class ProjectConfigFile:
    """Readable and writable project configuration file."""
    ...
    def data(self) -> dict[str, object]:
        return _deep_copy_mapping(self._data)

    def to_source(self) -> ConfigSource:
        return ConfigSource(name=str(self.path), values=_flatten(self._data))
...
def _flatten(data: Mapping[str, object], prefix: str = "") -> dict[str, object]:
    ...
    for key, value in data.items():
        ...
        if isinstance(value, Mapping):
            result.update(_flatten(_string_key_mapping(cast(Mapping[str, object], value)), dotted))
        else:
            result[dotted] = value
    return result


def _deep_copy_mapping(data: Mapping[str, object]) -> dict[str, object]:
    result: dict[str, object] = {}
    for key, value in data.items():
        if isinstance(value, Mapping):
            result[str(key)] = _deep_copy_mapping(
