...
from tinysoul.infra.continuation import (
    ContinuationError, ContinuationFailureReason, OpaqueContinuationCodec,
    continue_json_sequence,
)
from tinysoul.infra.json import JsonObject, JsonValue, dumps_json, to_json_object
from tinysoul.llm.messages import (
    AssistantMessage, JsonPart, Message, SystemMessage, TextPart, ToolResultMessage,
    UserMessage,
)
from tinysoul.llm.tools import ToolKind
from tinysoul.runtime import Signal

from .errors import ContextInvariantError, ContextInspectFailureReason, ContextInspectRequestError
from .segments import (
    ContextSegment, ReadOnlySegmentRegistration, RegisteredSegment, SegmentCapability, SegmentDescriptor,
    SegmentRegistration, SegmentReclaim, SegmentShape, SegmentSlot, TurnInfo,
)
from .signals import (
    SIGNAL_INPUT_APPEND, SIGNAL_TRACE_APPEND, SIGNAL_WORKING_PATCH, TraceAppend,
    parse_input_append_signal, parse_trace_append_signal, parse_working_patch_signal,
)
...
TRACE = SegmentDescriptor(
    "trace", "context", SegmentSlot.TRACE, 10,
    ref_prefixes=("turn:trace",), shape=SegmentShape.STACK,
...
class InputsSegment:
    ...
    async def prepare(self, updates: tuple[str, ...]) -> PendingInputs:
    ...
    def seal(self) -> JsonObject:
        return {"inputs": [
            {"id": item.input_id, "text": item.text, "received_at": item.received_at}
...
class TraceSegment:
    def __init__(self, trace: TurnTraceHeap, *, inspect_max_chars: int) -> None:
        ...
        self._trace_continuations = OpaqueContinuationCodec(owner="context", operation="inspect")

    async def prepare(self, updates: tuple[TraceAppend, ...]) -> TurnTraceHeap:
        candidate = deepcopy(self.state)
        for update in updates:
            if update.decision is not None:
                candidate.append_decision(update.decision, cycle_id=update.cycle_id, phase=update.phase)
            elif update.action_result is not None:
                candidate.append_action_result(
                    update.action_result, cycle_id=update.cycle_id,
                    canonical_message=update.canonical_action_result, origin_refs=update.origin_refs,
                )
            elif update.note is not None:
                candidate.append_phase_note(update.note, cycle_id=update.cycle_id, phase=update.phase)
        return candidate
    ...
    def reclaim(self, required_chars: int) -> SegmentReclaim:
        return SegmentReclaim(self.state.compact(required_chars=required_chars).reclaimed_chars)
        ...
        if structure.get("kind") != "context_trace_leaf":
            ...
            if len(dumps_json(structure)) > self._trace_inspect_max_chars:
                raise ContextInspectRequestError(
                    ...
                    "Context heap header exceeds the inspect character limit", constraint={"ref": ref},
                )
            return structure
        interactions = tuple(_semantic_trace_entry(entry) for entry in self.state.leaf_entries(ref))
        try:
            return continue_json_sequence(
                interactions, base={"kind": "context_trace_leaf", "ref": ref},
                item_field="interactions", continuation=continuation, codec=self._trace_continuations,
                ref=ref, max_chars=self._trace_inspect_max_chars,
            )
...
def core_registrations(
    *, identity: str, journal: str, inputs: InputsSegment, plan: PlanSegment, trace: TraceSegment,
) -> tuple[RegisteredSegment, ...]:
    return (
        ReadOnlySegmentRegistration(IDENTITY, CoreSegmentProvider(FixedTextSegment(identity, True))),
        ReadOnlySegmentRegistration(JOURNAL, CoreSegmentProvider(FixedTextSegment(journal))),
        SegmentRegistration(INPUTS, CoreSegmentProvider(inputs), SIGNAL_INPUT_APPEND, str, parse_input_append_signal),
        SegmentRegistration(PLAN, CoreSegmentProvider(plan), SIGNAL_WORKING_PATCH, WorkingPatch, _working_update),
        SegmentRegistration(TRACE, CoreSegmentProvider(trace), SIGNAL_TRACE_APPEND, TraceAppend, parse_trace_append_signal),
    )
