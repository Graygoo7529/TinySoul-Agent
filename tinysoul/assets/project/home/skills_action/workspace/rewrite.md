...
The Workspace enforces a bounded artifact size. Use `workspace.append` for additive text, `workspace.patch` for a localized replacement or larger document, and pass every source used for the rewrite through exact `reference_links`.
