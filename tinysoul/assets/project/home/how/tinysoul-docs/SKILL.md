...
1. Inspect the current Context for relevant Top Links and general HOW metadata.
2. Use the Phase1 `load_background` control with one or more exposed Top Links when their full bodies are needed.
3. Use `home.top.search` when a relevant WHAT, WHY, or general HOW Link has not already been exposed.
...
Detailed reference: <home:how/tinysoul-docs/references/use-tinysoul-context-and-link.md>.
