...
Recent entries are visible. Older entries may be represented by a `turn:trace@...` heap reference; use `core.context.inspect` only when their detail is needed. Inspect results return to the current Trace, not Background.
