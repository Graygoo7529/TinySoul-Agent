...
class SessionReconciler:
    ...
    def scan(self, manifest: SessionManifest) -> SessionReconcileScan:
        records = {record.ref: validate_turn_record(record) for record in self._store.list_records()}
        ...
        if len(indexed) != len(manifest.refs) or not indexed.issubset(records):
            raise SessionInvariantError("Session index contains duplicate or missing records")
