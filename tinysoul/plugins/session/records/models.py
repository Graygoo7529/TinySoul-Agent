...
class SessionInputRecord:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            or not isinstance(self.reply_to, str)
        ):
            raise SessionContractError("Session input identity is invalid")
...
class SessionActionRecord:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.result_id, str):
            raise SessionContractError("Session Action result identity must be text")
        ...
        if self.outcome is SessionActionOutcome.SUCCESS and self.failure is not None:
            raise SessionContractError("Successful Session Action cannot have failure")
        if (
            self.outcome in {SessionActionOutcome.FAILED, SessionActionOutcome.TIMEOUT}
            and self.failure is None
        ):
            raise SessionContractError("Failed Session Action requires failure facts")
        if self.outcome in {
            SessionActionOutcome.CANCELLED,
            SessionActionOutcome.NOT_EXECUTED,
            SessionActionOutcome.UNKNOWN,
        } and (self.failure is not None or self.result):
            raise SessionContractError(
                "Interrupted Session Action cannot fabricate a result"
            )
...
class SessionTurnRecord:
    ...
    def __post_init__(self) -> None:
        ...
        if (self.status is TurnOutcomeStatus.FAILED) != bool(
            self.failure or self.finish_failures
        ):
        ...
        if len(set(input_ids)) != len(input_ids) or len(set(result_ids)) != len(
            ...
            raise SessionContractError("Session occurrence identities must be unique")
        questions = {
            item.result_id
            for item in actions
            if item.action == "core.ask"
            and item.outcome is SessionActionOutcome.SUCCESS
        }
        if any(item.reply_to and item.reply_to not in questions for item in inputs):
            raise SessionContractError(
                "Session reply must refer to a successful question"
            )
    ...
    def from_json(cls, value: JsonObject) -> "SessionTurnRecord":
        ...
        return cls(
            ...
            failure=(
                _turn_failure(value["failure"])
                if value["failure"] is not None
                else None
            ),
            finish_failures=tuple(
...
def _turn_failure(value: object) -> TurnFailure:
    item = _exact_object(
        value,
        {"reason", "message", "module", "kind", "feedback"},
    ...
    try:
        return TurnFailure(
            ...
            module=module,
            kind=kind,
            feedback=_string_list(item["feedback"], "feedback"),
        )
...
def _require_day(value: str) -> None:
    try:
        ...
        raise SessionContractError("Session day must be an ISO date") from exc
...
def _optional_text(value: JsonObject, name: str) -> str:
    ...
    if not isinstance(item, str):
        raise SessionContractError(f"Session field must be text: {name}")
    return item
