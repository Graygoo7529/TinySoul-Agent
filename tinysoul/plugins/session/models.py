...
SESSION_RECORD_SCHEMA_VERSION = 8
...
class SessionInputRecord:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.input_id, str) or not self.input_id or not isinstance(self.reply_to, str):
    ...
    def to_json(self) -> JsonObject:
        return {"text": self.text, "received_at": float(self.received_at),
                "input_id": self.input_id, "reply_to": self.reply_to}

    @classmethod
    def from_json(cls, value: object) -> "SessionInputRecord":
        item = _exact_object(value, {"text", "received_at", "input_id", "reply_to"}, "Session input")
