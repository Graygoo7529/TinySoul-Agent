...
from tinysoul.kernel.registration import GenerationBuildContext, PluginConfig, PluginGeneration, PluginProfileExtension, PluginTurnResource, ProfileBuildContext, ProfileKind
from tinysoul.plugins.workspace.services import WorkspaceExecutionService
from tinysoul.runtime.events import EventFilter

from .config import SubagentSettings, parse_subagent_settings, validate_subagent_bindings
...
class SubagentPlugin:
    ...
    configuration = (PluginConfig(
        "capabilities.subagent", SubagentSettings, lambda tree, root: parse_subagent_settings(tree),
        RuntimeSubagentBridge().from_config_error, validate_subagent_bindings,
    ),)

    async def build_generation(self, context: GenerationBuildContext) -> PluginGeneration:
        engine = SubagentEngine(
            context.settings.get(SubagentSettings), jobs=context.services.get(JobRegistry),
            workspace=context.services.get(WorkspaceExecutionService), environment=context.runtime_env,
        )

        def extend(kind: ProfileKind, profile: ProfileBuildContext) -> PluginProfileExtension:
            return PluginProfileExtension(
                self.id, actions=partial(register_subagent_actions, engine=engine, scenario=kind.value),
                segments=(connections_registration(engine),),
                events=(TurnEventSubscription(
                    EventFilter(topic="subagent.connections", source="subagent"), connection_refresh,
        ...
        return PluginGeneration(self.id, profile_extension_factory=extend, sources=(engine,), release_day=release, close=close)
