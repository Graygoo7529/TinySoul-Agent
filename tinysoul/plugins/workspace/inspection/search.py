...
from ..storage.manifest import WorkspaceResourceRecord
...
class WorkspaceSearchScope:
    ...
    def __post_init__(self) -> None:
        ...
        if self.kind is WorkspaceSearchScopeKind.FILE:
            ...
            return
        if not self.locator.startswith(
            WORKSPACE_LINK_PREFIX
        ) or not self.locator.endswith("/"):
            raise WorkspaceContractError(
...
class WorkspaceSearchLineHint:
    ...
    def to_json(self) -> JsonObject:
        return {"link": self.link, "line": self.line}
    ...
    def search(
        ...
        *,
        ...
        if not isinstance(use_regex, bool):
            raise WorkspaceContractError("Workspace search regex flag must be boolean")
        try:
            pattern = (
                regex.compile(
                    normalized_query, 0 if case_sensitive else regex.IGNORECASE
                )
                if use_regex
                else None
            )
        ...
        for record in records:
            ...
            try:
                ...
                raise WorkspaceIOError("Workspace resource cannot be searched") from exc
            files_scanned += 1
            ...
            try:
                ...
                coverage_complete = False
                coverage_reason = "regex_timeout"
                break
            match_lines = tuple(matches)
            match_line_count += len(match_lines)
            stored_lines = match_lines[
                : max(0, self._settings.candidate_limit - len(compact_hints))
            ]
            compact_hints.extend(
                WorkspaceSearchLineHint(record.link, line) for line in stored_lines
            )
        ...
        *,
        ...
        for candidate in candidates:
            ...
            represented_lines.update(
                (candidate.link, line) for line in candidate.match_lines
            )
    ...
    *,
    ...
    return tuple(
        _fragment_for_window(
            ...
            tuple(window_matches),
            matches=matches,
            max_excerpt_chars=max_excerpt_chars,
        )
        for start, end, window_matches in windows
    )
...
def _fragment_for_window(
    ...
    *,
    ...
    match_span = matches.get(match_line)
