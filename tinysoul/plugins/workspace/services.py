...
class WorkspaceService(ScopedService[WorkspaceEngine]):
    def __init__(self, owner: WorkspaceEngine, scope: ServiceScope = ServiceScope()) -> None:
        ...
        self.prepare_analysis_references = scope.local(owner.prepare_analysis_references)
        self.prepare_edit_sources = scope.local(owner.prepare_edit_sources)
        self.commit_edit_text = scope.local(owner.commit_edit_text)
