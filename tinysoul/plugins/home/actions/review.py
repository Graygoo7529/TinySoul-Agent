...
from ..review import HomeReviewChange, HomeReviewResolution
...
HOME_REVIEW_ACTIONS = ("home.diff", "home.review")


class HomeReviewExecutor(ActionExecutor):
    ...
    async def execute(
        ...
        return await context.owner_operations.run_async(
            lambda: self._review(
                execution,
                replace(context, owner_operations=JoinedOperations()),
            )
        )
    ...
    async def _review(
        ...
        try:
            ...
            if execution.call.action_name == "home.diff":
                selected = tuple(
                    review
                    ...
                    for review in reviews:
                        ...
                        await home.resolve_review(
                            ...
                            (
                                resolution
                                if isinstance(review, HomeReviewChange)
                                else HomeReviewResolution.REJECT
                            ),
                        )
                    results.append(
...
def register_home_review_actions(
    builder: ActionEngineBuilder,
    *,
    controller: HomeReviewExecutor,
) -> ActionEngineBuilder:
    for handler in HOME_REVIEW_ACTIONS:
