...
from tinysoul.plugins.workspace.services import WorkspaceExecutionService, WorkspaceService
...
class ExecutionPlugin:
    ...
    configuration = (PluginConfig(
        "execution", ExecutionSettings, lambda tree, root: parse_execution_settings(tree),
    ...
    async def build_generation(self, context: GenerationBuildContext) -> PluginGeneration:
        engine = ExecutionEngine(
            settings=context.settings.get(ExecutionSettings), jobs=context.services.get(JobRegistry),
            workspace=context.services.get(WorkspaceExecutionService),
        )
        home, workspace = context.services.get(HomeService), context.services.get(WorkspaceService)

        def extend(kind: ProfileKind, profile: ProfileBuildContext) -> PluginProfileExtension:
            return PluginProfileExtension(
                self.id, requires=(HomeService, WorkspaceService),
                actions=partial(register_execution_actions, engine=engine, home=home, workspace=workspace),
            )
