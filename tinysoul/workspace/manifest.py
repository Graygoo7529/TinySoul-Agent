...
class WorkspaceResourceRecord:
    ...
    def from_json(cls, value: JsonObject) -> "WorkspaceResourceRecord":
        ...
        return cls(
            ...
            mtime=_required_number(value, "mtime"),
...
def _required_number(value: JsonObject, name: str) -> float:
    item = value.get(name)
    if isinstance(item, bool) or not isinstance(item, (int, float)):
        raise WorkspaceContractError(f"Manifest field must be a number: {name}")
    return float(item)
