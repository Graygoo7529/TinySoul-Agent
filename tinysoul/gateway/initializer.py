...
from .errors import ProjectInitializationError
from tinysoul.agent.catalog import builtin_action_catalog_files
...
class ProjectInitializationOutcome:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.root, Path) or not self.root.is_absolute():
            raise ProjectInitializationError("Initialized project root must be absolute")
        if (
            isinstance(self.file_count, bool)
            or not isinstance(self.file_count, int)
            or self.file_count <= 0
        ):
            raise ProjectInitializationError("Initialized project must contain files")
        if not isinstance(self.config_profile, ProjectConfigProfile):
            raise ProjectInitializationError("Initialized project config profile is invalid")
...
class ProjectResetOutcome:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.root, Path) or not self.root.is_absolute():
            raise ProjectInitializationError("Reset project root must be absolute")
        if (
            isinstance(self.file_count, bool)
            or not isinstance(self.file_count, int)
            or self.file_count <= 0
        ):
            raise ProjectInitializationError("Reset project must contain files")
        if not isinstance(self.config_profile, ProjectConfigProfile):
            raise ProjectInitializationError("Reset project config profile is invalid")
        if not isinstance(self.env_preserved, bool):
            raise ProjectInitializationError("Reset project env preservation state is invalid")
    ...
    def initialize(
        ...
        *,
        ...
        if not isinstance(target, Path):
            raise ProjectInitializationError("Project initialization target must be a path")
        if not isinstance(config_profile, ProjectConfigProfile):
            raise ProjectInitializationError("Project config profile is invalid")
...
class ProjectResetter:
    ...
    def reset(
        ...
        *,
        ...
        if not isinstance(target, Path):
            raise ProjectInitializationError("Project reset target must be a path")
        if not isinstance(config_profile, ProjectConfigProfile):
            raise ProjectInitializationError("Project config profile is invalid")
        expanded = target.expanduser()
        if expanded.is_symlink():
            raise ProjectInitializationError(f"Project reset target cannot be a symlink: {expanded}")
        root = expanded.resolve()
        ...
        try:
            ...
            raise ProjectInitializationError(f"Failed to reset TinySoul project: {exc}") from exc
        finally:
            if staging.exists():
                shutil.rmtree(staging, ignore_errors=True)
            if (
...
def _packaged_project_files(
    ...
    if not template.is_dir():
        raise ProjectInitializationError("Packaged TinySoul project template is missing")
    resources = (
        *_project_template_files(template, config_profile=config_profile),
        *_packaged_action_catalog_files(),
    )
    if not resources:
        raise ProjectInitializationError("Packaged TinySoul project template is empty")
    return resources


def _packaged_action_catalog_files() -> tuple[tuple[PurePosixPath, Traversable], ...]:
    return tuple(
        (PurePosixPath("configs/action/catalog") / path, resource)
        for path, resource in builtin_action_catalog_files()
    )
...
def _stage_project(
    ...
    *,
    ...
    if staging is None:
        raise ProjectInitializationError("Project staging directory was not created")
    return staging
...
def _project_template_files(
    ...
    *,
    ...
    if not any(relative.parts[0] == "configs" for relative in by_path):
        raise ProjectInitializationError("Packaged TinySoul config profile has no configs")
    if not any(relative.parts[0] == "home" for relative in by_path):
        raise ProjectInitializationError("Packaged TinySoul common template has no Home")
