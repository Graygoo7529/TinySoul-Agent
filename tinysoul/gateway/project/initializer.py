    ...
    def __post_init__(self) -> None:
        if not isinstance(self.root, Path) or not self.root.is_absolute():
            raise ProjectInitializationError(
                "Initialized project root must be absolute"
            )
        if (
        ...
        if not isinstance(self.config_profile, ProjectConfigProfile):
            raise ProjectInitializationError(
                "Initialized project config profile is invalid"
            )
...
class ProjectResetOutcome:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.env_preserved, bool):
            raise ProjectInitializationError(
                "Reset project env preservation state is invalid"
            )
    ...
    def initialize(
        ...
        *,
        ...
        if not isinstance(target, Path):
            raise ProjectInitializationError(
                "Project initialization target must be a path"
            )
...
class ProjectResetter:
    ...
    def reset(
        ...
        *,
        ...
        if expanded.is_symlink():
            raise ProjectInitializationError(
                f"Project reset target cannot be a symlink: {expanded}"
            )
        root = expanded.resolve()
        ...
        try:
            ...
            raise ProjectInitializationError(
                f"Failed to reset TinySoul project: {exc}"
            ) from exc
        finally:
            if staging.exists():
                shutil.rmtree(staging, ignore_errors=True)
...
def _packaged_project_files(
    ...
    assets = files("tinysoul.assets")
    common = assets.joinpath("common")
    profile = assets.joinpath(config_profile.value)
    if not common.is_dir() or not profile.is_dir():
        raise ProjectInitializationError(
            "Packaged TinySoul common or profile assets are missing"
        )
    resources = (*_template_files(common), *_template_files(profile))
    if not resources:
    ...
    for relative, resource in resources:
        if relative in by_path:
            raise ProjectInitializationError(
                f"Packaged project asset path is duplicated: {relative}"
            )
        by_path[relative] = resource
    required = {
        PurePosixPath(path)
        for path in ("tinysoul.toml", "README.md", ".gitignore", ".env.example")
    }
    if not required.issubset(by_path) or any(
        not any(path.parts[0] == directory for path in by_path)
        for directory in ("configs", "home")
    ):
        raise ProjectInitializationError(
            "Packaged TinySoul project template is incomplete"
        )
    return tuple(sorted(by_path.items(), key=lambda item: item[0].as_posix()))
