...
from tinysoul.gateway.endpoint import EndpointReady
from tinysoul.infra import atomic_write_text
from tinysoul.infra.json import JsonObject, dumps_json

from ..errors import ProjectInstanceError


@dataclass(frozen=True)
class ProjectInstanceIdentity:
...
class ProjectInstanceLease:
    ...
    def __init__(self, project_root: Path, *, directory: Path | None = None) -> None:
        ...
        self.identity = ProjectInstanceIdentity(
            project_root=root,
    ...
    def publish(self, ready: EndpointReady) -> None:
        if self._lock_file is None:
            raise ProjectInstanceError("Project instance lease is not active")
        if ready.instance_id != self.identity.instance_id or (
            ready.project_identity != self.identity.project_identity
        ):
            raise ProjectInstanceError(
                "Endpoint ready identity does not match its lease"
            )
        value: JsonObject = {
            "schema_version": 1,
...
def instance_directory() -> Path:
    ...
    if sys.platform == "darwin":
        return (
            Path.home() / "Library" / "Application Support" / "TinySoul" / "instances"
        )
