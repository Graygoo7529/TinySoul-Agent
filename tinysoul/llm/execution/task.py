...
class CapabilityPolicy:
    ...
    def missing_capabilities(
        ...
        return tuple(
            capability for capability in required if not model.supports(capability)
        )
...
class LLMTaskRunner:
    ...
    def current_model_capabilities(
        ...
        binding = next(item for item in bindings if item.provider_id == provider_id)
    ...
    async def _try_model(
        ...
        *,
        ...
        if last_error is None:
            raise LLMInvariantError("Provider chain failed without a provider error")
    ...
    async def _try_provider(
        ...
        *,
        ...
        if last_error is None:
            raise LLMInvariantError("Provider retry failed without a provider error")
        raise last_error
