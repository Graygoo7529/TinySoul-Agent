...
class AdapterSpec:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.api_style, ProviderApiStyle):
            raise LLMContractError("AdapterSpec.api_style must be a ProviderApiStyle")
    ...
    def to_json(self) -> JsonObject:
        result: JsonObject = {
            ...
            "common_option_keys": list(
                sorted(option.id for option in self.common_options)
            ),
...
_ADAPTER_SPECS: tuple[AdapterSpec, ...] = (
            ...
            (
                ...
                if key == "reasoning_keep"
                    ...
                    if key == "reasoning_summary"
                        ...
                        AdapterOptionSpec(key, "boolean")
                        if key == "store"
                        else (
                            AdapterOptionSpec(key, "number")
                            if key == "top_p"
                            else AdapterOptionSpec(key, "string")
                        )
                    )
                )
            )
    ...
    AdapterSpec(
        ...
        common_options=(
            AdapterOptionSpec(
                "reasoning_keep", "enum", ("none", "content"), "reasoning_keep"
            ),
        ...
        protocols=(
            AdapterProtocolSpec(
                "k2",
                (AdapterOptionSpec("thinking", "string", ("enabled", "disabled")),),
            ),
            AdapterProtocolSpec(
                "k3", (AdapterOptionSpec("reasoning_effort", "enum", ("max",)),)
            ),
        ),
    ),
    AdapterSpec(
            ...
            (
                ...
                if key in {"do_sample"}
                    ...
                    if key == "top_p"
                        ...
                        AdapterOptionSpec(key, "object", validator="thinking_glm")
                        if key == "thinking"
                        else (
                            AdapterOptionSpec(key, "enum", ("none", "content"))
                            if key == "reasoning_keep"
                            else (
...
def _validate_option_value(spec: AdapterOptionSpec, value: object, *, key: str) -> None:
    if spec.validator == "thinking_deepseek":
        _validate_thinking_object(
            value, key=key, allowed_types={"enabled", "disabled"}, object_keys={"type"}
        )
        return
    if spec.validator == "thinking_glm":
        _validate_thinking_object(
            ...
            object_keys={"type", "clear_thinking"},
        )
        return
    if spec.choices and value not in spec.choices:
        raise ConfigError(
            "Adapter option value is invalid",
            key=key,
            value=value,
    ...
    if spec.value_kind == "boolean" and not isinstance(value, bool):
        raise ConfigError(
            "Adapter option must be a boolean", key=key, value=value, expected="bool"
        )
    if spec.value_kind == "number" and (
        isinstance(value, bool) or not isinstance(value, (int, float))
    ):
        raise ConfigError(
            "Adapter option must be a number", key=key, value=value, expected="float"
        )
...
def _validate_thinking_object(
    ...
    *,
    ...
    if not isinstance(value, Mapping):
        raise ConfigError(
            "Adapter thinking option has an invalid type", key=key, value=value
        )
    object_value = {str(name): item for name, item in value.items()}
    raw_type = object_value.get("type")
    if raw_type not in allowed_types:
        raise ConfigError(
            "Adapter thinking type is invalid", key=f"{key}.type", value=raw_type
        )
    for name, item in object_value.items():
        if name not in object_keys:
            raise ConfigError(
                "Unknown adapter thinking option", key=f"{key}.{name}", value=item
            )
    if "clear_thinking" in object_value and not isinstance(
        object_value["clear_thinking"], bool
    ):
        raise ConfigError(
            "Adapter clear_thinking must be a boolean",
            key=f"{key}.clear_thinking",
            value=object_value["clear_thinking"],
        )
