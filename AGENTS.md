...
R5 Gateway v2 与 Endpoint 生命周期收口已完成，见 `docs/analysis/done/20260920-done-Agent重构第五轮子计划-Gateway v2与SDK协议闭环.md`、`docs/analysis/done/20260920-done-Agent重构第五轮收口子计划-Endpoint生命周期与重启.md`：HTTP、CLI 与 SDK 共用 AgentCommands 和 owner facade，Turn/Job/Runtime 只通过类型化投影，旧 `/v1` 路径删除，Observation replay 使用 instance/cursor/gap。HTTP 不提供项目 reset，Job 停止不越过 Turn owner；稳定 EndpointHost 保持 instance/journal/cursor，在 HTTP restart 中只重绑 Agent generation，旧 facade 失效后由调用者重新获取，ACP Job 应答移交 S6。Agent.wait 跨 restart，CLI 重新获取配置与 commands；真实 HTTP server 连续重启、失败重试与 Ctrl-C 已验证。2026-09-21 Windows Fast 1104 passed、28 deselected，Full 1109 passed、23 deselected（含全部 5 项 Generation/wheel），typecheck 与 diff-check 通过；未运行 Linux 实机和真实 provider/network。主计划 S5 标记 done，S3 延后项和 S6–S7 保留。
